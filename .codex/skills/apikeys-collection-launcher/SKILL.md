...
- For UI/UX work, use `docs/UI_UX_DEVELOPMENT_CONTRACT.zh-TW.md` as the contract before implementing non-trivial UI changes. Product references such as Foxy, Steam, or `tem/` are behavioral inspiration only; do not copy their naming into code, docs, or UI text unless the user explicitly approves that name.
...
- Provider/source discovery UI/CLI must remain metadata-only. Writing `provider_candidates.ui.json` or a provider candidate source-draft summary is a review handoff, not catalog promotion, install, credential probing, or source ingestion. If a UI or CLI action writes a selected provider candidate forward, it may only write ignored local provider discovery seed config or, when a supported crawler type and endpoint are explicit or conservatively inferred, ignored local dataset discovery source config. Source-draft summary and structured events should include `next_action` / `audit_command`, and `--handoff-report` should surface the next local discovery dry-run audit command for cross-machine agents. Official catalog changes still require later crawler audit and promotion guards.
- Crawler success is not just "no exception". Zero candidates, suspiciously low counts, duplicate-only output, duplicate-heavy output, missing evidence URLs, or unexpected payload shape should create warnings/errors. Prefer machine-readable `warning_codes`, `next_action`, and top-level `audit_summary` for crawler audit output so UI/agents can route to query/parser repair, dedupe/id-mapping review, metadata mapping repair, or normal candidate review without parsing warning prose. Tk warning surfaces, including local-discovery promotion dry-runs, should show the aggregate `audit_summary` first, then bounded per-source details.
