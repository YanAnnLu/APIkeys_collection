...
- For repeatable MVP smoke checks, prefer `--write-mvp-demo-flow state/mvp_demo/flow.json`: it writes a flow manifest, Socrata adapter-review plan, offline JSON fixture, and offline direct plan. Use the offline plan to prove `download -> manifest -> SQLite import` without network, and the Socrata resolved plan to check the real bounded adapter path. If adding a UI entry later, call `api_launcher.mvp_demo` instead of duplicating demo business logic inside `launcher_ui.py`.
- For download/import orchestration, use `api_launcher.ingestion_pipeline.run_download_import_slice()` as the service boundary. Do not make new UI panels, subcommands, or agents call `run_download_plan_payload()` directly unless they are intentionally testing the low-level runner.
