...
- MySQL/PostgreSQL connection probes can use `information_schema` helpers for table counts and schema fingerprints when drivers/env vars are available. Table asset ownership and existence checks across those engines are still the next backend task.
