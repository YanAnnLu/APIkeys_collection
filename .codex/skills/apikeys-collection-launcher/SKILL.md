...
- Adapter review payloads include `outcome_bucket` and summary `by_outcome`. Use these machine-readable buckets, such as `source_resolution_required` and `downloaded_payload_transform`, to group follow-up work; do not infer adapter next steps only from UI labels or free-text reasons.
