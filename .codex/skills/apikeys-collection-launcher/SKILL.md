...
- Use `--run-download-plan-json` with `--run-download-plan` when an agent needs machine-readable pipeline status, stage, `next_action`, skip buckets, import counts, and errors instead of parsing human CLI lines.
...
- For repeatable MVP smoke checks, prefer `--write-mvp-demo-flow state/mvp_demo/flow.json` or Tk `工具 > 產生 MVP Demo Flow`: both call `api_launcher.mvp_demo`, write a flow manifest, Socrata adapter-review plan, agent-readable adapter review JSON, offline JSON fixture, and offline direct plan. Use the offline plan to prove `download -> manifest -> SQLite import` without network, and the Socrata resolved plan to check the real bounded adapter path. Do not duplicate demo business logic inside `launcher_ui.py`.
