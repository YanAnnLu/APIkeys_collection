...
| 2 | `api_launcher/crawlers/dataset_sources.py` | 共用型別已移到 `api_launcher/crawlers/types.py`，共用 metadata helper 已移到 `metadata.py`，HTTP/JSON/URL helper 已移到 `fetch.py`，full-crawl page cap 與去重 append helper 已移到 `pagination.py`，NCEI、ERDDAP、HTML index parser，以及 STAC/CMR/CKAN/GBIF/Dataverse/Zenodo parser/pagination flow 已移到各自模組；目前主要保留 dispatcher、少量 URL builder 與 NCEI pagination flow | 把剩下的 NCEI pagination flow 拆到 `ncei.py`，讓 orchestrator 與 source parser 邊界更清楚。 |
