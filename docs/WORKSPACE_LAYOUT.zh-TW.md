...
| `frontends/tk/` | 目前 Tk 控制台 | UI 過渡期仍保留，但業務邏輯要慢慢外移到 `api_launcher/`。`launcher_ui.py` 應只保留 widget、事件與視窗生命週期；純 helper 先放 `ui_helpers.py`，Provider table view-model 先放 `provider_models.py`。Dialog implementations 現在應放在 focused owner modules，`dialogs.py` 只保留相容 re-export facade。 |
...
| 2 | `api_launcher/crawlers/dataset_sources.py` / `registry.py` | 共用型別已移到 `api_launcher/crawlers/types.py`，共用 metadata helper 已移到 `metadata.py`，HTTP/JSON/URL helper 已移到 `fetch.py`，full-crawl page cap 與去重 append helper 已移到 `pagination.py`，各 source 的 query URL builder/parser/source-level fetch/parse flow/pagination flow 已移到各自模組；source type dispatch 已收斂到 `api_launcher/crawlers/registry.py` 的 `CrawlerSpec` / matrix / capability code，`dataset_sources.py` 只保留相容入口與 `SOURCE_CRAWLER_HANDLERS` 診斷/相容 surface | 後續再新增 crawler type 時，請先登記 registry spec 與測試，不要把 if/elif 鏈加回來；保留 `discover_dataset_candidates_for_source()` 的相容入口。 |
