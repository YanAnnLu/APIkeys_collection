...
| `api_launcher/crawlers/` | 資料集發現爬蟲 | `types.py` 放共用資料結構，`metadata.py` 放共用 metadata 判斷，`stac.py`/`ckan.py`/`erddap.py`/`cmr.py` 是已拆出的 source parser；上層 orchestrator 統一調度。 |
...
| 2 | `api_launcher/crawlers/dataset_sources.py` | 共用型別已移到 `api_launcher/crawlers/types.py`，共用 metadata helper 已移到 `metadata.py`，STAC、CKAN、ERDDAP、CMR parser 已移到各自模組；其他 source parser 仍在同檔 | 繼續依 source type 拆成 repository index / repository search 等模組，再由 orchestrator 調度。 |
...
這次已先做幾個小拆分：`api_launcher/cli_flags.py` 集中判斷「是否有 CLI 指令被指定」，讓 `core.py` 不再維護一大段旗標清單；`api_launcher/crawlers/types.py` 集中 crawler 共用資料結構；`api_launcher/crawlers/metadata.py`、`api_launcher/crawlers/stac.py`、`api_launcher/crawlers/ckan.py`、`api_launcher/crawlers/erddap.py`、`api_launcher/crawlers/cmr.py` 先把共用 metadata helper、STAC collection parser、CKAN package parser、ERDDAP allDatasets parser、NASA CMR collection parser 從 `dataset_sources.py` 拆出來。後續拆 GBIF、Dataverse、Zenodo 或 HTML index 時，請沿用這種小步、可測、保留舊匯入路徑的方式。
