...
```powershell
L:\RRKAL_project\scripts\heartbeat_check.cmd
```

準備接外部 agent 時，改用 dry-run agent 模式：

```powershell
L:\RRKAL_project\scripts\heartbeat_agent.cmd
```
...
Arguments: /c L:\RRKAL_project\scripts\heartbeat_codex.cmd
Start in: L:\RRKAL_project
```
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File L:\RRKAL_project\scripts\heartbeat_agent.ps1 -RunAgent -AgentExecutable "YOUR_AGENT_EXE" -AgentArguments @("ARG1", "ARG2")
```
