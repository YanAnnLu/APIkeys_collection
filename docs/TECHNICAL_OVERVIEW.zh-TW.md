...
QR 登入比手動填 API key 更適合一般使用者，但 token 儲存必須謹慎：不可寫進 Git、不可放在一般設定檔、需要本機 private token store 或系統 credential vault。目前架構是「profile 裡放登入規格，token 放 `state/private`」；Google OAuth Client ID 可由 UI 的 `設定 QR 登入` 寫入本機 `launcher_integrations.local.json`，也仍支援從 `GOOGLE_OAUTH_CLIENT_ID` 環境變數讀取。若某個 AI 服務沒有 OAuth device-code 端點，就先走 API key，不能硬造不存在的 QR flow。
...
Tk UI 中可以到 `設定 > AI 輔助模型` 選擇要用的 profile；雲端 profile 可在同一頁用「設定 QR 登入」保存 OAuth Client ID、「登入選取模型」開 QR/device 登入，或用 session API key 入口貼 key。Google/Gemini 專用入口仍保留，但它只負責登入/token，不會自動切換目前使用的 AI profile。

UI 也有原生選單列：

- `Integrations > Google / Gemini login and AI`
- `Integrations > Google QR login`
