...
| Google browser OAuth | MVP | `整合 > Google 帳號登入（瀏覽器）` 會打開系統瀏覽器，使用 authorization-code + PKCE + `127.0.0.1` loopback callback，讓 Google 頁面處理選帳號、手機確認或掃碼登入。 |
...
Google 帳號瀏覽器登入是一般使用者主路線；QR/device-code 是進階備用情境。token 儲存必須謹慎：不可寫進 Git、不可放在一般設定檔、需要本機 private token store 或系統 credential vault。目前架構是「profile 裡放登入規格，token 放 `state/private`」；Google OAuth Client ID 可由 UI 的 `設定 OAuth Client ID` 寫入本機 `launcher_integrations.local.json`，也仍支援從 `GOOGLE_OAUTH_CLIENT_ID` 環境變數讀取。若某個 AI 服務沒有官方 OAuth / device-code 端點，就不能硬造不存在的 QR flow。
...
Tk UI 中可以到 `設定 > AI 輔助模型` 選擇要用的 profile；雲端 profile 可在同一頁用「設定 OAuth Client ID」保存 app 身分、「用帳號登入選取模型」開瀏覽器 OAuth，或用 session API key 入口貼 key。Google/Gemini 專用入口仍保留，但它只負責登入/token，不會自動切換目前使用的 AI profile。

UI 也有原生選單列：

- `Integrations > Google / Gemini login and AI`
- `Integrations > Google account login (browser)`
- `Integrations > Advanced: Google QR / device-code login`
