...
```mermaid
flowchart LR
    A[Scientific dataset source] --> B[Launcher download / manifest / registry]
    B --> C[Renderer-ready cache]
    C --> D[Taichi reference renderer]
    C --> E[Unreal preview exporter]
    E --> F[UE Content/APIkeysCollection/Preview]
    F --> G[Unreal import / virtual twin scene]
```
...
```mermaid
flowchart LR
    A[Raw datasets] --> B[Launcher registry]
    B --> C[Normalized tile builder]
    C --> D[Tile manifest with dataset id/version/lod]
    D --> E[Local tile service or file-backed tile index]
    E --> F[Unreal runtime streamer]
    F --> G[Camera-driven LOD selection]
```
