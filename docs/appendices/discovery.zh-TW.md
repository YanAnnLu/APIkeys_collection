...
若 `adapter_required` 來自 CKAN/Data.gov 類平台，且 plan 只拿到 `package_show` URL，或只有 `package_search` URL 加 dataset id，`--resolve-adapter-plan` 現在可以只查一次單一 package metadata，再從 resources / `distribution` / `distributions` / `dcat:distribution` / JSON-LD `@graph` 裡挑安全的 direct file。命名空間欄位如 `dcat:downloadURL`、`dct:format`、`dcat:byteSize` 也可被正規化；`accessURL` 這類可能只是入口頁的欄位仍留在 review。這仍然是 bounded adapter：它不掃整個 CKAN catalog，也不下載 HTML 頁或大型未知資源。
