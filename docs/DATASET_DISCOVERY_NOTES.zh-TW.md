...
目前 `catalog/dataset_discovery_sources.json` 內建 22 個可爬資料目錄。支援的通用 crawler 類型：
...
- NASA CMR：把 collection concept id 轉成 `granules.json?page_size=1` 的 JSON metadata sample，不抓背後的衛星科學資料檔。
- Socrata/SODA：把 resource/API view 轉成 `$limit=25` 的 JSON/CSV/GeoJSON sample。
- NOAA/NCEI Search / Access Data：只在 query 已有 dataset、時間、空間邊界時產生小樣本；否則留在 adapter review。
