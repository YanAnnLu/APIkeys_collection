...
- `CrawlerCapabilityProfile` 也會把 `seed_scope` 放進 crawler asset payload。這個欄位描述 handler 的「seed 枚舉表面」；它不是目前 listing 是否已列完的證據。前端若要顯示遠端完整度，仍必須讀 listing result 的 `seed_enumeration` 與 `remote_pagination`。
