...
- `--crawler-run-summary-json` 是 crawler run 狀態的輕量 CLI：只掃 structured event log，不重跑 crawler，不讀完整 resolved plan，輸出與 handoff / Web Preview 共用的 latest listing / plan build summary。
