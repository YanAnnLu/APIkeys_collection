...
Web Preview 的推薦 seed 面板另有「驗證閉環」按鈕。這個動作會先重新跑該 crawler asset 的 bounded listing，讀回本機 seed page，再使用後端 `recommended_seed_uid` 觸發正式 seed-level download/import。它適合用來快速確認一個入口是否能完成「入口 -> seed -> 下載 / 匯入」小閉環；如果來源缺登入/API Key，畫面會先停在「記住我的帳號」設定流程，不會直接送出必然失敗的 live request。
...
1. 進入「爬蟲資產」分頁，選一張 crawler asset 卡片。
   ...
   - 按 `開 Seed 表格 / 下載` 會開啟本頁 seed 表格。選一筆 seed 後，可以切換收藏、按 `探測欄位` 先把 seed URL 交給 schema/head probe，或按 `下載此 Seed` 走正式 seed-level download/import service。Tk 不會在表格裡重新爬遠端，也不會自行猜 provider 規則。
2. 按「送進下載器」或同等下載計畫動作。
