...
`--handoff-report` 會列出目前作用中 data-store profile、對應測試指令、JSON 測試指令與 env 範本指令；它不會代替使用者測連線，也不會輸出密碼。
...
若這一步是給 heartbeat 或 agent 接力判讀，請加 `--run-download-plan-json`，stdout 會改成 JSON，包含 `stage`、`next_action`、`result.skip_summary`、下載/匯入統計與 errors；人類預設摘要仍維持原本的 `[download-plan] ...` 文字。
