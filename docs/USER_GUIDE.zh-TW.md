...
`--handoff-report` 會列出目前作用中 data-store profile、對應測試指令、JSON 測試指令與 env 範本指令；也會顯示最近一次 MVP demo smoke 的 `stage`、成功狀態、table 與 `row_count` 摘要。它不會代替使用者測連線，也不會輸出密碼。若要給 heartbeat 或外部 agent 直接解析同一份交接 snapshot，使用 `--handoff-report-json`，stdout 會是純 JSON，不會混入 `[db]`、`[seed]` 或 `[handoff]` 人類提示。
