...
Seed 清單中的「下載此 seed」會呼叫正式 seed 下載 / 匯入路徑。Web 會走 `POST /api/crawler-assets/{asset_id}/seed-download-import`；Tk 會從右側 Crawler Passport 開「Seed 表格 / 下載」，選取 seed 後呼叫同一個後端 service。後端會驗證該 seed 是否屬於目前 crawler asset，再從 catalog seed 建立 resolved plan。它不會重新打遠端 crawler，也不是舊「真下載示範」；若 seed 沒有可直接下載的 URL、需要憑證或需要 adapter review，畫面應依後端回傳的 `plan_outcome` / `next_action` 顯示下一步。Web seed row、Tk Seed 表格與 Tk Crawler Passport 摘要會先顯示後端 `content_import_profile` 產生的匯入路徑，例如可匯入 SQLite、下載後需解壓或轉換、內容 Parser 待辦，讓使用者在按下載前就知道這筆 seed 大致會進哪條處理路徑。若 seed page 回傳 `recommended_seed_uid`，Web 列表上方會出現「下載推薦 seed」，Tk Seed 清單 dialog 也會顯示「推薦 seed」摘要並自動選中推薦列；這個推薦由後端挑選，使用者不需要先猜哪筆 seed 最適合第一次測試下載。

Web Preview 的推薦 seed 面板另有「驗證閉環」按鈕。這個動作會先重新跑該 crawler asset 的 bounded listing，讀回本機 seed page，再使用後端 `recommended_seed_uid` 觸發正式 seed-level download/import。Tk Seed 清單 dialog 也提供同名入口，走同一個後端 closure service。它適合用來快速確認一個入口是否能完成「入口 -> seed -> 下載 / 匯入」小閉環；如果來源缺登入/API Key，畫面會先停在「記住我的帳號」設定流程，不會直接送出必然失敗的 live request。

如果界域表單需要資料欄位，例如時間欄位或要保留的 columns，請先用 seed row 的「探測欄位」。Web 會呼叫 `POST /api/crawler-assets/{asset_id}/bounds-form/schema-probe` 後重繪同一份 bounds form；Tk Seed 清單 dialog 會在背景呼叫同一個 schema probe service，然後開啟「爬蟲界域設定」視窗。成功時，`time_field`、`columns` 這類欄位會盡量變成下拉或勾選選單；失敗時，UI 會顯示警告並保留原本表單，使用者仍可套用推薦值或手動輸入。這個動作只探測欄位並產生 bounds payload，不會直接下載資料。

同一條 seed-level formal download/import 也可以用 CLI 驗收，方便展示前或 agent 自動化檢查：

```powershell
py -B APIkeys_collection.py --db state\crawler_seed_cli\launcher.sqlite --init-db --seed --run-crawler-seed-download-import ASSET_ID DATASET_UID --crawler-seed-download-import-json
```

CLI 會使用與 Web/Tk 相同的後端 service：從本機 catalog seed 建立 resolved plan，寫出 seed plan artifact，執行 bounded direct download/import，並輸出純 JSON。若要指定輸出位置，可搭配既有 `--downloads-root` 與 `--import-sqlite-db`；展示與 smoke 建議使用本機 Downloads、Temp 或本地 clone，不要把 live SQLite import 預設壓在雲端同步路徑。

若要讓 agent 或展示前 smoke 自動跑「入口 -> 推薦 seed -> 下載匯入」的小閉環，可以使用 recommended seed closure 指令：

```powershell
py -B APIkeys_collection.py --db state\live_closure\launcher.sqlite --init-db --seed --run-crawler-asset-recommended-seed-closure sf_open_data_socrata_catalog --crawler-asset-closure-json --downloads-root state\live_closure\downloads --import-sqlite-db state\live_closure\curated.sqlite
```

這個指令會先 live 枚舉 crawler asset，讀本機 seed page，再使用後端 `recommended_seed_uid` 觸發正式 seed-level download/import。它不是舊「真下載示範」，也不是前端自己猜 seed；若沒有推薦 seed、缺 provider、listing 被封鎖或下載匯入未完成，JSON 會回報 `closure_stage` 與 `next_action`。

Tk 版在啟動 seed 下載前也會先做本機登入檢查。若來源需要登入或 API Key，但目前尚未設定完整，Tk 會先停下並開啟「登入設定 / 記住我的帳號」視窗。這個視窗會顯示官方登入 / 申請 API Key 入口、需要貼上的欄位、目前是否已設定，以及「清除已保存」與「記住我的帳號」勾選。留空代表不變，不會預填已保存的明文金鑰；儲存後才會交給後端本機 credential service 更新本機忽略檔或目前執行環境。它不會直接啟動背景下載 thread 去送出必然失敗的請求。Tk 爬蟲資產表格也會顯示「登入」欄，右側 Crawler Passport 會列出登入狀態、已設定欄位數、缺少欄位與下一步，方便在按下載前先看出哪個入口需要補登入設定。
...
1. 進入「爬蟲資產」分頁，選一張 crawler asset 卡片。
   ...
   - 按 `開 Seed 表格 / 下載` 會開啟本頁 seed 表格。選一筆 seed 後，可以切換收藏、按 `探測欄位` 先把 seed URL 交給 schema/head probe，或按 `下載此 Seed` 走正式 seed-level download/import service。若頁面有後端 `recommended_seed_uid`，也可按 `驗證閉環`，讓 Tk 重新跑 bounded listing、讀回本機 seed page，再下載 / 匯入後端推薦 seed。Tk 不會在表格裡自行猜 provider 規則或挑 seed。
2. 按「送進下載器」或同等下載計畫動作。
3. 如果該爬蟲支援界域，系統會依後端 `bounds_schema` 動態產生表單，例如 limit、bbox、time range、collection、format。
   ...
   - 這些快速值只來自後端 `CrawlerAssetBoundFormSpec`，UI 不會自行猜資料集版本、collection、時間欄位或 bbox。若要讓時間欄位與欄位選擇更精準，先在 Seed 表格中按 `探測欄位`，讓後端 schema/head probe 把可見 seed 的欄位回填到同一份表單。
