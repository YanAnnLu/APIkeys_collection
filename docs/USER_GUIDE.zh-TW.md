...
CLI 會使用與 Web/Tk 相同的後端 service：從本機 catalog seed 建立 resolved plan，寫出 seed plan artifact，執行 bounded direct download/import，並輸出純 JSON。若要指定輸出位置，可搭配既有 `--downloads-root` 與 `--import-sqlite-db`；展示與 smoke 建議使用本機 Downloads、Temp 或本地 clone，不要把 live SQLite import 預設壓在 K 槽雲端同步路徑。
