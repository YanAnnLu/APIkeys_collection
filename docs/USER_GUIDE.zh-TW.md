...
開發者若要在瀏覽器確認 crawler handler contract，可讀 `GET /api/diagnostics/crawler-handler-smoke`。這個 endpoint 只回傳 compact summary 與 `developer_only=true`，不包含每個來源的完整 smoke report，也不代表 live endpoint 可用；一般使用者下載資料不需要操作它。
