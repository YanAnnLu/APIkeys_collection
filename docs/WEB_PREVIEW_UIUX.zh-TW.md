...
- Crawler source type 的顯示文字也應來自後端 payload：`CrawlerCapabilityProfile` / `CrawlerAsset.to_dict()` 會提供 `source_type_label`；Web filter、Passport、Downloader row、selected hero 顯示 label 或「來源範式待確認」。raw `source_type` id 只保留給 JSON/debug、filter key、search haystack 與路由，不作為使用者主要文案。
