...
- Download/import 結果應優先顯示後端 `download_import.stage_label`。Web 端可保留中性 fallback，但正式文案 ownership 在 `api_launcher.crawler_asset_display`，不要在 JS 端長期維護一份 stage 翻譯表。
