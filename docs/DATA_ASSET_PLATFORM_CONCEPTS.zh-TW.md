...
```mermaid
flowchart LR
    A[外部 Source] --> B[Raw Artifact<br/>HTML / JSON / CSV / PDF / API response]
    B --> C[Parsed Dataset<br/>表格 / 地理 / 時間序列 / 影像]
    C --> D[Standardized Dataset<br/>manifest / schema / quality / processing log]
    D --> E[Storage Backend<br/>SQLite / DuckDB / PostGIS / Lakehouse]
    E --> F[Derived Dataset<br/>feature / aggregate / prediction / render cache]
    F --> G[Output<br/>Query / Render / ML / Report]
```
...
```mermaid
flowchart TB
    T[Discovery Tool Registry] --> C1[CKAN Searcher]
    T --> C2[CMR Searcher]
    T --> C3[OGC Records Reader]
    T --> C4[Socrata Searcher]
    T --> C5[Notion Intake Sync]
    C1 --> O[Discovery Orchestrator]
    C2 --> O
    C3 --> O
    C4 --> O
    C5 --> O
    O --> D[Candidate Dataset]
    D --> R[Adapter Review]
    R --> P[Download / Import Plan]
    P --> M[Manifest + Standardized Asset]
```
...
```mermaid
flowchart LR
    S[外部資料來源] --> A[Crawler / Adapter]
    A --> P[Download / Import Plan]
    P --> B[Lakehouse Staging / Bronze]
    B --> M[Raw Manifest]
    M --> Q[Schema + Quality Check]
    Q --> X[Deduplicate / Standardize / Convert]
    X --> SG[Silver Table]
    SG --> G[Gold Data Product]
    G --> C[Catalog + Lineage]
    C --> U[Query / Render / ML]
```
...
```mermaid
flowchart TB
    Data[資料本體] --> Push[Lakehouse Push Manifest]
    Source[Source Metadata<br/>provider / source_url / version / license] --> Push
    Processing[Processing Metadata<br/>rename / parse / dedupe / convert] --> Push
    Quality[Quality Metadata<br/>row_count / nulls / checksum / status] --> Push
    Target[Target Metadata<br/>profile / layer / table / partition] --> Push
    Lineage[Lineage Metadata<br/>crawler / adapter / job / output] --> Push
    Push --> Lake[Lakehouse Catalog / Table]
```
...
```mermaid
flowchart TB
    Core[APIkeys Collection<br/>Data Asset Control Plane]
    Core --> D[Discovery Connectors<br/>Crawler / CKAN / CMR / Notion]
    Core --> S[Storage Connectors<br/>SQLite / PostGIS / Lakehouse]
    Core --> R[Renderer Connectors<br/>Matplotlib / TradingView-like / VisPy / Blender]
    Core --> C[Control Connectors<br/>K8S / Spark / Airflow / Argo]
    Core --> W[Collaboration Connectors<br/>Notion / GitHub / Slack]
    Core --> M[Model Connectors<br/>MLflow / DVC / Feature Store]
```
...
```mermaid
flowchart TB
    UI[Desktop UI<br/>桌面控制台] --> Agent[Local Agent<br/>下載 / 匯入 / 環境檢查 / renderer 呼叫]
    Mobile[Mobile Companion<br/>狀態查看 / 安全指令] --> API[Backend API<br/>catalog / profile / job / team sync]
    UI --> API
    Agent --> API
    API --> Runtime[Remote Runtime<br/>K8S / Spark / Cloud Worker]
    API --> Storage[Storage Backends<br/>SQLite / DuckDB / Object Storage / Lakehouse]
    Runtime --> Storage
    Agent --> Local[Local Assets<br/>state / downloads / SQLite / render cache]
```
