...
本專案現在明確區分兩種工作模式，未來複製到其他專案時也應保留這個分流。

| 模式 | 觸發語境 | 主要目標 | 可降低的顆粒度 | 不可降低的底線 | 收尾動作 |
| --- | --- | --- | --- | --- | --- |
| 日常開發模式 | 一般推進、重構、MVP hardening、測試補強、文件整理 | 穩健實現正式產品功能 | 速度可以慢一點，切片可以更小 | 測試、契約、文件、可維護邊界與安全 guard | 更新 GTD/handoff/docs，必要時走 OpenSpec，保留測試證據 |
| 快速交付 / 展示模式 | 展示、會議、給組員看、現場操作、中午/今晚前、只剩 X 分鐘/小時、需要 GUI/PPT/講稿 | 讓人類展示者不用改程式碼也能重現真實能力 | 功能範圍、樣本大小、SQL/全來源/長期轉接器深度 | 真資料或明確 fixture、真進度、真輸出、可驗證 GUI/PPT/檔案、明確 fallback | 壓力測試後把短路徑回收成正式任務、測試、OpenSpec/GTD 與 skill/handoff 規則 |

快速交付不是脫離工程紀律，而是另一種工程紀律。它允許「先求可演示」，但不允許「假裝已完成」。如果展示切片短路到本機下載資料夾、使用可控樣本、或避開 SQL/MySQL/PostgreSQL，必須在 UI、講稿或 handoff 中說清楚。若遠端 API timeout，fallback 也必須明示來源與原因。

日常開發模式恢復後，Agent 應停止新增展示專用表面功能，改做三件事：確認急改沒有破壞主線、列出急改暴露出的正式需求、把可重用切片拆回防禦性編程路線。這能避免快速交付變成一次性補丁，也能讓壓力測試真正提高產品成熟度。
