...
| Unreal Engine bridge | Skeleton | Local UE 5.7 install detected; local `APIkeysVirtualTwin.uproject` created outside repo; preview exporter writes GEBCO/HYG-derived OBJ/MTL/CSV plus bridge manifest into `Content/APIkeysCollection/Preview`; docs now define Unreal as the final camera-driven tile-streaming frontend. | Add Unreal-side auto-import/editor utility, then implement tile manifest schema and runtime LOD streamer. |
