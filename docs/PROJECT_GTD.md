...
| Dataset adapters | Skeleton | Adapter registry exists and HYG v3.8 star catalog can be discovered into the dataset table from the HYG provider. | Add GEBCO, NOAA/NASA, and ERDDAP dataset adapters with download manifests. |
...
| Renderer bridge | Skeleton | Renderer contracts, GEBCO/HYG dataset IDs, copied `taichi_global_bathymetry.py`, renderer requirements documented; HYG dataset discovery now uses the shared renderer contract ID. | Map installed dataset IDs to renderer-ready file paths and test with real assets. |
...
| Documentation | In progress | `docs/ARCHITECTURE.md`, `docs/TECH_STACK.md`, `docs/TECHNICAL_OVERVIEW.zh-TW.md`, `docs/DATASET_DISCOVERY_NOTES.zh-TW.md`, renderer notes, GTD tracker, handoff docs exist. | Repair/normalize Chinese overview encoding and keep Chinese and English docs aligned as features move from skeleton to MVP. |
