...
| Dataset freshness/versioning | Planned | Risk identified: seeds and adapters can point to older compatibility-pinned data rather than the latest provider version; GEBCO 2025 is now marked as compatibility-pinned while GEBCO 2026 requires review. | Add version freshness checks, latest-version hints, and stale/compatibility-pinned UI badges. |
