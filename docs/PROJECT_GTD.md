...
| Download repair scanner | Skeleton | CLI `--verify-downloads`, `--manifest-health`, and `api_launcher/repair.py` can scan sidecar manifests, detect missing payloads/size mismatch/checksum mismatch/manifest parse errors, and sync health status into SQLite. | Add UI repair panel and automatic requeue suggestions. |
