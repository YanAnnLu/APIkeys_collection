...
| Unreal Engine bridge | Skeleton | Local UE 5.7 install detected; config supports Unreal project profiles; environment checks cover engine/editor/project/content; CLI `--unreal-bridge-plan` can map render bridge assets to `/Game/APIkeysCollection/...`; Chinese design note exists. | Create/select a real `.uproject`, then implement asset sync/copy and Unreal-side import strategy. |
