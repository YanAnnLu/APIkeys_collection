...
- 選中資產的 hero 與右側資產護照也會呈現 `latest_plan_outcome` 摘要；JS 只渲染後端的 label/tone/counts/content-review badge，不依 outcome bucket 寫前端業務分支。下一步若要更正式，應把 resolved-plan passport / asset profile 狀態放到後端 service，而不是在 Web session 裡累積狀態。
