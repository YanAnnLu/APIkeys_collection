...
- Crawler 不能只看「程式沒報錯」。如果抓到 0 筆、低於預期、全是重複、重複候選異常偏高、payload shape 不符，都要明確 warning/error；使用者特別在意這種假成功。新 crawler audit 應盡量提供 `warning_codes` 與 `next_action`，讓 UI/agent 能判斷要修搜尋詞、修 parser、檢查去重/id mapping，或進入候選審核。
