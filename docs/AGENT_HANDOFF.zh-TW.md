...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端實質 checkpoint 是 `365d937 Split discovery Tk workflows`；GitHub Actions run `26335997043` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第二十一輪安全解耦：把 provider candidate review panel、provider discovery worker/message、crawler audit summary/issue helpers、dataset candidate discovery worker 與本機 discovery dry-run audit 抽成 `DiscoveryWorkflowMixin`；provider/dataset discovery backend、review JSON、`audit_summary` 與 event log 行為不變。`launcher_ui.py` 已降到 1861 行。仍沒有搬主表格/detail 控制、AI summary UI 或資料庫 repair backend。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `365d937` 已驗證。`py_compile` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_helpers tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_provider_models tests.test_discovery tests.test_discovery_drafts tests.test_discovery_promotion -v` 共 77 tests OK；`git diff --check` / cached check OK；第一次 `.\scripts\pre_push_smoke_brief.cmd` 在 `git rev-parse @{u}` 前遇到 Windows/Git cwd transient error，隨即重跑通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26335997043` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整通過 log 在 `state/logs/pre_push_smoke_20260523_225827.log`，不提交。 |
