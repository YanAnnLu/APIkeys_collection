...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-24 目前已確認的遠端實質 checkpoint 是 `2b579de Split Tk source action workflows`；GitHub Actions run `26342472725` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的主檔解耦收束：把 Treeview row 事件、右鍵 library action menu、repair context、star toggle、metadata self-check、納管/解除納管/受控卸載、下載計畫自檢、metadata crawl worker 與官方文件開啟流程抽成 `SourceActionWorkflowMixin`。`launcher_ui.py` 已降到 202 行，現在主要保留 app state 初始化、translation wrappers、相容匯出與 Tk entrypoint。 |
...
| 本輪本機驗證 | 2026-05-24 Windows/K：UI 解耦 checkpoint `2b579de` 已驗證。temp `PYTHONPYCACHEPREFIX` `py_compile` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_helpers tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_provider_models -v` 共 62 tests OK；`git diff --check` / cached check OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26342472725` Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260524_041159.log`，不提交。 |
...
10. 下一階段可以轉向 Backend/Core Hardening，但要先用 OpenSpec 定義第一個小切片，不要一次做五項：`core.py`/CLI physical split、`yfinance` adapter registry/pluginization、`db.py` lightweight migration runner、crawler dynamic discovery、credential template rules catalog 化。
