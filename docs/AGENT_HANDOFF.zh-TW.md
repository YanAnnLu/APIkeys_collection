...
| 上次本機驗證 | 2026-05-20 macOS/Antigravity/CloudMounter：IDE 更新後目前這個舊 Codex session 仍被 `CODEX_SANDBOX=seatbelt`、`CODEX_SANDBOX_NETWORK_DISABLED=1` 限制；`~/.codex/config.toml` 已是 `sandbox_mode = "danger-full-access"` / `approval_policy = "never"`，但本 session 沒吃到，建議關閉 IDE / reload window / 開新 Codex session 後先驗證環境變數。餘波修復：`APIkeys_collection.py` 曾被工作樹刪除，已用 HEAD 可讀內容補回；缺失 Git blob `a60de8185d6ad444079a35ede668b19b1e70c5fa` 經確認等於 `frontends/unreal/README.zh-TW.md` 工作樹 hash 後，用 `git hash-object -w` 補回。功能驗證：`PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m py_compile api_launcher/import_policies.py frontends/tk/launcher_ui.py tests/test_tk_ui_preferences.py` OK；`PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m unittest tests.test_download_plan_runner tests.test_tk_ui_preferences -v`，9 tests OK；sandbox 內 full test 因 seatbelt 禁止 `127.0.0.1` 測試伺服器 bind 而會報 `PermissionError: Operation not permitted`，升權後重跑 `PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m unittest discover -s tests -v`，335 tests OK、skipped=1（optional numpy renderer dependency）；`git diff --check` OK；GitHub Actions run `26144606542` Windows/Ubuntu success。該 run 有 notice：`windows-latest` requests 會在 2026-06-15 前導到 `windows-2025-vs2026`；目前不是失敗，後續若 CI runner warning 變多再調整 workflow runner。 |
| 最近新增重點 | Tk UI guided import 現在會在「匯入可支援下載結果」前詢問同名 SQLite table 處理方式：預設「保留舊表，匯入成新表」，也可選「保留舊表，略過同名項目」或「覆蓋同名表」；覆蓋會二次確認。這把 CLI 既有 `skip` / `rename` / `replace` policy 接到 UI，保持 beginner-safe 預設不覆蓋。UI 也會把最後選擇的策略存到 ignored local integration config 的 `ui_import_existing_table_policy`，並在下載計畫面板顯示目前匯入策略；無效偏好值會回到安全 rename。`tests.test_download_plan_runner` 也補了 replace regression：明確 replace 會重建同名表，不會保留舊資料或偷偷改名。上一個 checkpoint：Plan-driven import CLI 可用 `--plan-import-existing-table-policy rename` 把新結果匯入 `table_2` 這類新表，或明確用 `replace` / `--import-replace-table` 覆蓋。 |
| MVP 剩餘估算 | 約 12-13%；bounded adapter 的 DOI/DataCite、NOAA/NCEI data-file、CMR granule data-link、常見 resource 欄位命名與 JSON-LD `@graph` 包裝風險各收掉一塊，database self-check UI 已有 profile/schema 修正、停止追蹤單一壞資產與 manifest-backed missing SQLite table reimport，plan-driven import 重跑現在有預設略過、CLI/UI safe-rename、明確 replace 三種路徑；剩下主要是擴大 guarded repair 範圍、crawler source 類型收束、以及少量 UI polish / docs sync |
