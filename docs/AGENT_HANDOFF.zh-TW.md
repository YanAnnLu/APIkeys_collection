...
- Crawler 共用 metadata helper 已拆到 `api_launcher/crawlers/metadata.py`，HTTP fetch/JSON/URL helper 已拆到 `api_launcher/crawlers/fetch.py`，共用 full-crawl page cap 與候選去重 append helper 已拆到 `api_launcher/crawlers/pagination.py`，STAC collection payload parser 與 STAC pagination flow 已拆到 `api_launcher/crawlers/stac.py`，CKAN package payload parser 與 CKAN pagination flow 已拆到 `api_launcher/crawlers/ckan.py`，ERDDAP `allDatasets` payload parser 已拆到 `api_launcher/crawlers/erddap.py`，NASA CMR collection payload parser 與 CMR pagination flow 已拆到 `api_launcher/crawlers/cmr.py`，GBIF dataset search payload parser 與 GBIF pagination flow 已拆到 `api_launcher/crawlers/gbif.py`，Dataverse search payload parser 與 Dataverse pagination flow 已拆到 `api_launcher/crawlers/dataverse.py`，Zenodo records payload parser 與 Zenodo pagination flow 已拆到 `api_launcher/crawlers/zenodo.py`，HTML file index parser 已拆到 `api_launcher/crawlers/html_index.py`，NCEI search payload parser 與 NCEI pagination flow 已拆到 `api_launcher/crawlers/ncei.py`。`dataset_sources.py` 目前主要保留 dispatcher 與少量 non-full query glue；後續若要再瘦身，可把各 source 的非 full-crawl URL 組裝也搬進各自模組。
...
- 第 1 項目前已調整為「善用 crawler 發現 provider/source 與 dataset candidates」，不要把每個代表資料集都硬寫成 Python adapter。`catalog/APIkeys_collection_catalog.json` 目前有 48 個 provider seed，新增方向包含 NOAA GOES-R on AWS、NOAA NOMADS、Marine Regions、GADM、OpenStreetMap Overpass、U.S. Census TIGERweb、EMODnet ERDDAP、Harvard Dataverse、Zenodo、Canada/UK/Australia/HDX CKAN。`catalog/dataset_discovery_sources.json` 描述可爬的資料目錄，目前 17 個 source；`api_launcher/crawlers/orchestrator.py` 統一調度 source crawlers，並行執行、去重、收斂 per-source error/warning；`api_launcher/crawlers/fetch.py` 已放共用 HTTP fetch/JSON/URL helper；`api_launcher/crawlers/pagination.py` 已放共用 full-crawl page cap 與候選去重 append helper；`api_launcher/crawlers/stac.py` 已放 STAC collection payload parser 與 STAC pagination flow；`api_launcher/crawlers/ckan.py` 已放 CKAN package payload parser 與 CKAN pagination flow；`api_launcher/crawlers/erddap.py` 已放 ERDDAP `allDatasets` payload parser；`api_launcher/crawlers/cmr.py` 已放 NASA CMR collection payload parser 與 CMR pagination flow；`api_launcher/crawlers/gbif.py` 已放 GBIF dataset search payload parser 與 GBIF pagination flow；`api_launcher/crawlers/dataverse.py` 已放 Dataverse search payload parser 與 Dataverse pagination flow；`api_launcher/crawlers/zenodo.py` 已放 Zenodo records payload parser、Zenodo pagination flow 與檔案摘要 helper；`api_launcher/crawlers/html_index.py` 已放 HTML file index parser；`api_launcher/crawlers/ncei.py` 已放 NOAA/NCEI Search payload parser 與 NCEI pagination flow；`api_launcher/crawlers/dataset_sources.py` 目前主要保留 dispatcher 與少量 non-full query glue。Crawler 審核不能只看「沒報錯」：0 筆候選、低於 `min_expected_candidates`、只抓到全域重複候選、payload shape 不符、候選缺少 evidence/source url 都要提示或失敗。未來新增供應商時，優先新增/配置 crawler，由 orchestrator 調度；不要讓特殊網頁邏輯散進 UI 或 core。AIS 與衛星雲圖是代表測試案例：AIS 應由 MarineCadastre index 發現 shards，衛星雲圖應由 NOAA/NCEI/GOES-R/Earth Engine/STAC 類 catalog 發現 raster/grid 候選。
