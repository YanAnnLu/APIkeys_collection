...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端 checkpoint 是 `6805ea9 Split Google Gemini settings dialog`；GitHub Actions run `26326541203` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第九輪安全解耦：把 Google/Gemini 連線視窗抽成 `GoogleGeminiSettingsDialog` class，主檔只保留委派呼叫，實際 OAuth browser/device-code、Gemini API key 儲存與 credential 操作仍留在既有主 UI 方法以降低風險。`launcher_ui.py` 已降到約 5427 行。仍沒有搬 OAuth flow 實作、repair panel、下載/匯入 backend 或資料庫 repair backend。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `6805ea9` 已驗證。`py_compile` 使用 temp `PYTHONPYCACHEPREFIX` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_ui_helpers tests.test_tk_provider_models -v` 共 59 tests OK；`git diff --check` OK；`.\scripts\pre_push_smoke_brief.cmd` 共 507 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26326541203` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260523_150144.log`，不提交。 |
