...
- Provider/source discovery UI 入口也要保持 metadata-only。`資料庫 > 發現 provider 候選` 只寫 `state/provider_candidates.ui.json` 供 review，不正式納管、不安裝、不抓 API key；正式 catalog 仍應經 local config / crawler audit / promotion guard。
