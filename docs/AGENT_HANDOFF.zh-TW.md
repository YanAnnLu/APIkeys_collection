...
- 已驗證：`py -3 -B -m py_compile frontends\web\server.py tests\test_web_preview.py` OK；`py -3 -B -m unittest tests.test_web_preview.WebPreviewApiTest.test_real_download_demo_route_is_developer_diagnostic_only -v` OK；`py -3 -B -m unittest tests.test_web_preview tests.test_crawler_assets -v` 86 tests OK；`py -3 -B -m unittest tests.test_tk_dialogs tests.test_developer_diagnostics -v` 73 tests OK；docs/frontends-web mojibake scan OK；`git diff --check` OK（僅 CRLF/LF warning）；`.\scripts\pre_push_smoke_brief.cmd` 通過，847 tests / 4 skipped，MVP smoke `download_import_completed` / `row_count=3`，log：`state\logs\pre_push_smoke_20260528_174707.log`。仍待 commit/push 與 CI。
