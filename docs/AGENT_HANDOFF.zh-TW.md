...
- Tk UI 新增 `設定 > AI 輔助模型`，使用者必須明確選擇要調用哪個 AI profile；目前支援 local Ollama、Gemini、OpenAI-compatible profile。AI 登入/API key 可各自存在 profile/session，但生成描述時以此設定為準。每個 profile 可帶 `oauth_device` 設定；支援 OAuth 的服務會把 token 存在 `state/private/ai_oauth_tokens/`。
- Tk UI 的 Gemini/Google dialog 已改成帳號登入導向：主按鈕是 `用 Google 帳號登入`，會開系統瀏覽器做 authorization-code + PKCE + loopback callback；Google 頁面負責選帳號、手機確認或掃碼登入。API key 只保留成備用入口，不要再把它當作對一般使用者的推薦路線。
- Tk UI 現在有 `整合 > Google 帳號登入（瀏覽器）` 直達入口，也保留 `整合 > 進階：Google QR / 裝置碼登入`。若缺 Google OAuth Client ID，UI 會先開 `Google 登入前置設定` 導引頁，說明這不是 Gmail / 密碼 / API key，再讓使用者貼上 Client ID 並寫入 `config/launcher_integrations.local.json`；格式不像 `*.apps.googleusercontent.com` 的值會被拒絕保存，避免重複觸發 Google `invalid_client`。`api_launcher/oauth_device.py` 仍保留環境變數 fallback。
...
1. 做 dataset candidate review/import UI，讓使用者能審核 crawler 找到的資料集，再排入下載/安裝計畫。
2. 繼續擴充 crawler source 類型，但要維持設定檔驅動；下一批可評估 OGC API Records、Dataverse、Socrata、OpenAlex/DataCite 類 metadata 來源。
...
4. 擴充 repair 建議到 adapter-specific datasets，並把 download/database JSON repair payload 接到更完整事件 log 與 UI guided repair flows。
...
8. 擴充 `.codex/skills/apikeys-collection-launcher`，加入 crawler/dataset review、adapter-specific repair/download/database self-check 操作流程。
