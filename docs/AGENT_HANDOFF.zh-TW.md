...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪最近功能 checkpoint 是 dev requirements 加入 `numpy`，讓 Unreal preview export 測試在一般開發/CI 安裝後實際執行而非 skipped；push 後請用 `gh run list --branch main --limit 5` 與 `gh run watch ... --exit-status` 確認 CI。 |
| 上次本機驗證 | 2026-05-20 Windows/RaiDrive：dependency/test-environment checkpoint 使用本機磁碟 venv `C:\Users\lyn59\AppData\Local\APIkeys_collection\venv-py313\Scripts\python.exe` 驗證；`-B -m py_compile scripts\export_unreal_preview.py` OK；`-B -m unittest tests.test_unreal_preview_export -v`，2 tests OK；`-B -m unittest discover -s tests -v`，331 tests OK、skipped=0；`git diff --check -- requirements-dev.txt docs\AGENT_HANDOFF.zh-TW.md docs\SETUP.zh-TW.md docs\TECH_STACK.md` OK。上一個功能 checkpoint：JSON-LD `@graph` resource metadata 使用 `.venv\Scripts\python.exe -B -m py_compile api_launcher\adapter_plan_resolver.py` OK；adapter resolver 43 tests OK；full 331 tests OK、當時因未裝 `numpy` skipped=1；CI run `26132764678` success。注意：本機 `python` 指到 WindowsApps 佔位程式；雲端磁碟寫 `__pycache__` 或 `.venv` 套件檔可能卡住，測試時可用 `-B`、設 `PYTHONDONTWRITEBYTECODE=1`，或改用本機磁碟 venv/pycache。 |
