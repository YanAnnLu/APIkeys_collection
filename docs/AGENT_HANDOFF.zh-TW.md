...
- 已完成本地驗證：in-memory Python syntax check OK；`node --check frontends\web\static\app.js` OK；`py -3 -B -m unittest tests.test_crawler_asset_download tests.test_tk_dialogs tests.test_web_preview -v` 103 tests OK；docs mojibake scan OK；`git diff --check` OK；`.\scripts\pre_push_smoke_brief.cmd` 通過，811 tests / 4 skipped，MVP demo smoke `download_import_completed` / `row_count=3`，log：`state\logs\pre_push_smoke_20260528_043200.log`。尚未 push / CI；下一步請 commit/push 並觀察 GitHub Actions。
