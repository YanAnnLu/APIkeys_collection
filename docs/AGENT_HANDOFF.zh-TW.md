...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪最近功能 checkpoint 是 CMR `feed.entry` JSON importer support；push 後請用 `gh run list --branch main --limit 5` 與 `gh run watch ... --exit-status` 確認 CI。 |
| 上次本機驗證 | 2026-05-19 macOS：CMR `feed.entry` JSON importer checkpoint 驗證 `PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache conda run -n metal_trade_312 python -m unittest tests.test_json_importer tests.test_adapter_plan_resolver tests.test_dataset_download_plan`，41 tests OK；`python -m py_compile api_launcher/importers/json_importer.py tests/test_json_importer.py` OK；`python -m unittest discover -s tests`，299 tests OK；`git diff --check` OK。前一個 CMR live smoke：`--discover-dataset-candidates --dataset-discovery-source nasa_earthdata_cmr_collections --dataset-discovery-limit 1 --upsert-dataset-candidates --db state/ci_cmr_catalog.sqlite --summary` 抓到 6 candidates、0 errors、0 warnings；再匯出 1 筆 candidate plan，`--resolve-adapter-plan` 轉成 `cmr_bounded_granule_search_resolver`，URL 為 `/search/granules.json?collection_concept_id=...&page_size=1`，`import_plan=supported_after_download`。OpenSpec tooling checkpoint 也已驗證 `npx -y @fission-ai/openspec@latest validate --all --no-interactive`、`conda run -n metal_trade_312 pyside6-designer` 路徑存在、Spectra 安裝於 `~/Applications/Spectra.app`。上一輪 live smoke：OpenAlex 抓到 10 candidates、0 errors、0 warnings；NOAA/NCEI live smoke 以 `daily-summaries` + station + 1 日 JSON 查詢取回 1 筆；CLI Socrata smoke 抓到 10 candidates、0 errors、0 warnings；另一次 end-to-end Socrata smoke 走完 discover candidates -> export candidate plan -> resolve `$limit=25` plan -> run download plan -> verify manifests -> import SQLite，結果 completed=1、failed=0、imported=1 |
| 最近新增重點 | `api_launcher/importers/json_importer.py` 現在支援 NASA CMR 常見的 `feed.entry` 巢狀陣列，讓 CMR bounded granule metadata sample 能被攤平成 SQLite rows；`tests/test_json_importer.py` 已新增 CMR feed fixture regression。既有重點：CMR collection candidate 會轉成 `/search/granules.json?collection_concept_id=...&page_size=1` JSON metadata sample，且 `cmr_collection` 不再因 `.json` 副檔名被誤判為 direct file；OpenSpec CLI 已初始化 `openspec/`，Spectra GUI 已安裝到 `~/Applications/Spectra.app`，Qt Designer 已確認在 `metal_trade_312`；workspace inventory CLI、`docs/WORKSPACE_LAYOUT.zh-TW.md`、handoff portal/local discovery summary、crawler shared modules 與 source-level fetch/parse flow 小拆分已完成；crawler source type dispatcher 已集中成 `SOURCE_CRAWLER_HANDLERS` mapping，portal intake 共用同一份 `SUPPORTED_DATASET_SOURCE_TYPES`；跨平台接力檢查已併入本文件 |
...
- `api_launcher/adapter_plan_resolver.py` 現在也能把 NASA CMR collection candidate 轉成 bounded `page_size=1` granule metadata JSON plan entry。這只下載一筆 granule metadata，讓衛星/地球觀測來源先走通 JSON manifest/import 小閉環；不會直接下載 HDF/NetCDF/COG 等科學資料資產。`plans.py` 也會把 `cmr_collection` 視為 adapter-required，避免 `.json` API endpoint 被誤當成普通 direct file。
