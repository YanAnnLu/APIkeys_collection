...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端 checkpoint 是 `01e20d0 Expose source draft audit handoff`；GitHub Actions run `26322747363` 成功。這個 checkpoint 讓 provider candidate source draft summary 帶 `next_action`、`audit_command`、`audit_source_ids`，CLI 也會寫入 `provider_candidate_source_drafts_written` event；`--handoff-report` / `--handoff-report-json` 會顯示最近一次 source draft 寫入與下一步 local discovery dry-run audit 指令。正式 catalog 仍需 crawler audit / promotion guard。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：`py -3 -B -m unittest tests.test_discovery_drafts tests.test_discovery tests.test_discovery_promotion tests.test_launcher_ui -v` 共 50 tests OK；`git diff --check` OK；`scripts\pre_push_smoke_brief.cmd` 共 482 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26322334358` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260523_112553.log`，不提交。 |
