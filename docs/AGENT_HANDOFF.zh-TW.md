...
- `--crawler-run-summary-json` 是更輕的狀態查詢入口，只讀 structured event log 並輸出最新 listing / download-plan build summary。若任務只需要 crawler run counts / next_action，先用這個；若還需要 catalog、GTD、MVP readiness，再讀完整 `--handoff-report-json`。
