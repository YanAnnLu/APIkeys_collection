...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪最近功能 checkpoint 是 OGC API Records broker-link guard；push 後請用 `gh run list --branch main --limit 5` 與 `gh run watch ... --exit-status` 確認 CI。 |
| 上次本機驗證 | 2026-05-19 macOS：OGC Records broker-link guard checkpoint 驗證 `PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache conda run -n metal_trade_312 python -m unittest tests.test_dataset_discovery tests.test_dataset_download_plan tests.test_adapter_plan_resolver`，58 tests OK；`python -m py_compile api_launcher/crawlers/ogc_records.py tests/test_dataset_discovery.py` OK；WMO live smoke 仍抓到 5 candidates、0 errors、0 warnings、seeded providers=54；匯出 WMO candidate plan 後，`dataset_version.download_url` 退回 HTTP(S) catalog query，不再使用 `mqtts://...` broker link 當主要 URL。前一個 WMO source checkpoint：catalog JSON 格式 OK；`python -m unittest discover -s tests`，299 tests OK；`git diff --check` OK。前一個 CMR checkpoint：`tests.test_json_importer tests.test_adapter_plan_resolver tests.test_dataset_download_plan`，41 tests OK；CMR live smoke 抓到 6 candidates 並能轉成 `cmr_bounded_granule_search_resolver` 的 `/search/granules.json?collection_concept_id=...&page_size=1`。OpenSpec tooling checkpoint 也已驗證 `npx -y @fission-ai/openspec@latest validate --all --no-interactive`、`conda run -n metal_trade_312 pyside6-designer` 路徑存在、Spectra 安裝於 `~/Applications/Spectra.app`。 |
| 最近新增重點 | `api_launcher/crawlers/ogc_records.py` 現在只會用 HTTP(S) `data/download/items/self` links 當 dataset primary `api_url`；WMO/WIS2 這類 `mqtts://` broker/notification links 仍保留在 metadata `links`，但不再被放到主要下載/adapter URL，避免後續計畫看起來像要直接下載 broker stream。前一個重點：`catalog/APIkeys_collection_catalog.*` 新增 WMO WIS2 Global Discovery Catalogue provider，`catalog/dataset_discovery_sources.json` 新增 `wmo_wis2_gdc_records` OGC API Records source；`api_launcher/importers/json_importer.py` 支援 NASA CMR 常見的 `feed.entry` 巢狀陣列；CMR collection candidate 會轉成 `/search/granules.json?collection_concept_id=...&page_size=1` JSON metadata sample。OpenSpec CLI 已初始化 `openspec/`，Spectra GUI 已安裝到 `~/Applications/Spectra.app`，Qt Designer 已確認在 `metal_trade_312`；crawler source type dispatcher 已集中成 `SOURCE_CRAWLER_HANDLERS` mapping，portal intake 共用同一份 `SUPPORTED_DATASET_SOURCE_TYPES`；跨平台接力檢查已併入本文件 |
