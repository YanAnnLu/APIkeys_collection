...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪最近功能 checkpoint 是 resource metadata key normalization；push 後請用 `gh run list --branch main --limit 5` 與 `gh run watch ... --exit-status` 確認 CI。 |
| 上次本機驗證 | 2026-05-20 Windows/CloudMounter：resource metadata key normalization checkpoint 使用 `.\.venv\Scripts\python.exe -B -m py_compile api_launcher\adapter_plan_resolver.py` OK；`.\.venv\Scripts\python.exe -B -m unittest discover -s tests -p test_adapter_plan_resolver.py -v`，41 tests OK；`.\.venv\Scripts\python.exe -B -m unittest discover -s tests -v`，329 tests OK、skipped=1；`git diff --check -- api_launcher\adapter_plan_resolver.py tests\test_adapter_plan_resolver.py docs\AGENT_HANDOFF.zh-TW.md docs\DATASET_DISCOVERY_NOTES.zh-TW.md docs\PROJECT_GTD.md docs\PROJECT_STATE.md docs\TECH_STACK.md docs\USER_GUIDE.zh-TW.md docs\appendices\discovery.zh-TW.md` OK。注意：本機 `python` 指到 WindowsApps 佔位程式，請用 `.venv\Scripts\python.exe`；雲端磁碟寫 `__pycache__` 可能卡住，測試時可用 `-B`、設 `PYTHONDONTWRITEBYTECODE=1`，或把 pycache 指到本機暫存。前一個 checkpoint：plan-driven import 重跑時既有 SQLite table 會記為 `skipped_existing_table`，不當成失敗，也不覆蓋資料。 |
| 最近新增重點 | Generic adapter-plan resource resolver 現在會辨識常見 resource 包裝 `resources` / `distribution` / `distributions` / `links`，以及 direct-link 欄位 `downloadURL` / `downloadUrl` / `contentUrl` / `contentURL` / `fileUrl` / `fileURL`，欄位值可為字串、list，或 `{"@id": "..."}` 這類 JSON-LD 物件；也能讀 `mediaType` / `contentType` / `encodingFormat` 格式提示，以及 `byteSize` / `contentSize` / `{"@value": "..."}` 大小提示。這讓 DCAT/schema.org-like metadata 在已明確提供檔案 URL、格式與大小時，可以走既有 `--resolve-adapter-plan` / UI `解析 Adapter 計畫` 入口變成 direct plan entry；只有 `{"label": "CSV download"}` 這類純標籤物件不會被誤當成網址，仍留在 adapter review。不需要為單一欄位命名或 JSON-LD 包裝差異新增 provider adapter。前一個重點：`--run-download-plan ... --import-supported-plan-results` 重跑時，既有 SQLite table 會記為 `skipped_existing_table`，不當成失敗，也不覆蓋資料。 |
| MVP 剩餘估算 | 約 15-16%；bounded adapter 的 DOI/DataCite、NOAA/NCEI data-file、CMR granule data-link 與常見 resource 欄位命名風險各收掉一塊，database self-check UI 已有 profile/schema 修正、停止追蹤單一壞資產與 manifest-backed missing SQLite table reimport，plan-driven import 重跑也不再把已存在 table 誤判為失敗；剩下主要是擴大 guarded repair 範圍、crawler source 類型收束、少量 import policy / UI polish |
