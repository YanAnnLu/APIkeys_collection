...
- 已補 regression：OpenAlex cursor、DataCite `links.next`、Zenodo `links.next`、CMR page number 都有 page-cap `has_more` 測試。已驗證：`py -B -m unittest tests.test_dataset_discovery` 45 tests OK；`py -B -m unittest tests.test_dataset_discovery tests.test_crawler_assets tests.test_crawler_seed_registry tests.test_web_preview` 129 tests OK；相關 crawler 檔案 py_compile OK；docs mojibake scan OK；`git diff --check` OK；`.\scripts\pre_push_smoke_brief.cmd` 772 tests / 4 skipped，MVP demo smoke `download_import_completed` / `row_count=3`。下一步仍需 commit / push 與 CI watch。
