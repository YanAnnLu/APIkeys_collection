...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪功能 checkpoint 是 real database smoke CI service coverage：新增獨立 GitHub Actions `real-db-smoke` job，用 disposable MySQL/PostgreSQL service containers 跑 opt-in 真實 driver 測試；一般 Windows/Ubuntu test matrix 不需要 DB driver。push 後請用 `gh run list --repo YanAnnLu/APIkeys_collection --branch main --limit 5` 與 `gh run watch RUN_ID --repo YanAnnLu/APIkeys_collection --exit-status` 確認 CI。 |
| 上次本機驗證 | 2026-05-20 macOS/Antigravity/CloudMounter：IDE 更新後目前這個舊 Codex session 仍被 `CODEX_SANDBOX=seatbelt`、`CODEX_SANDBOX_NETWORK_DISABLED=1` 限制；`~/.codex/config.toml` 已是 `sandbox_mode = "danger-full-access"` / `approval_policy = "never"`，但本 session 沒吃到，建議關閉 IDE / reload window / 開新 Codex session 後先驗證環境變數。餘波修復：`APIkeys_collection.py` 曾被工作樹刪除，已用 HEAD 可讀內容補回；缺失 Git blob `a60de8185d6ad444079a35ede668b19b1e70c5fa` 經確認等於 `frontends/unreal/README.zh-TW.md` 工作樹 hash 後，用 `git hash-object -w` 補回。本輪驗證：CI workflow YAML parse OK；real-driver smoke test `py_compile` OK；`tests.test_data_store_real_drivers` 2 tests skipped as intended；`tests.test_data_store_connections tests.test_data_store_real_drivers tests.test_database_self_check` 60 tests OK、skipped=2；`PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m unittest discover -s tests -v`，344 tests OK、skipped=3（2 個 opt-in real DB smoke、1 個 optional numpy renderer dependency）；`git diff --check` OK。上一個 opt-in smoke checkpoint 已由 GitHub Actions run `26147585557` Windows/Ubuntu success 驗證；本輪新增的 service-container smoke 需要 push 後看新的 `real-db-smoke` job。 |
| 最近新增重點 | Archive bounded transform 現在支援 ZIP/TAR 內的 `csv.gz`、`json.gz`、`jsonl.gz`、`ndjson`、`ndjson.gz`、`geojson.gz`，會寫衍生 manifest 並接既有 CSV/JSON importer；regression 已覆蓋 ZIP 內 `sample.ndjson.gz` 與 TAR.GZ 內 `sample.geojson.gz` 匯入 SQLite。Registry/source provenance 也正式接受並保存 compound source format，例如 `csv.gz`、`jsonl`、`ndjson`、`geojson`、`geojson.gz`、`zip`、`tar.gz`；下載檔案 asset 與 CSV/JSON/GeoJSON 匯入後的 curated table asset 會記錄實際 payload format。Guarded SQLite table reimport 也會用同一組支援格式，且錯誤/UI 文案會列出支援清單。本輪新增 `requirements-db-smoke.txt` 與 CI `real-db-smoke` job，讓 opt-in real MySQL/PostgreSQL smoke tests 在受控 service containers 裡真正連線驗證；一般本機/Windows/Ubuntu全量測試仍預設不連線、不 DROP、不覆蓋既有 table、也不猜測未記錄來源。 |
| MVP 剩餘估算 | 約 8-9%；bounded adapter 的 DOI/DataCite、NOAA/NCEI data-file、CMR granule data-link、常見 resource 欄位命名與 JSON-LD `@graph` 包裝風險各收掉一塊，archive transform 已可處理 ZIP/TAR 內常見 CSV/JSON/GeoJSON 變體，database self-check UI 已有 profile/schema 修正、停止追蹤單一壞資產與 manifest-backed missing SQLite table reimport，且 guarded reimport 已支援 CSV/JSON/JSONL/GeoJSON 類匯入來源；database issue JSON 現在只會在缺失 SQLite table 同時具備記錄 manifest 與支援格式時標 `can_auto_repair=true`；real MySQL/PostgreSQL smoke tests 已有 opt-in 覆蓋並接進受控 CI service containers；plan-driven import 重跑現在有預設略過、CLI/UI safe-rename、明確 replace 三種路徑；剩下主要是把 real-driver smoke 擴到 registry-backed self-check 場景、擴大更多明確擁有權 repair 範圍、crawler source 類型收束、以及少量 UI polish / docs sync |
...
2. 擴充 SQL/database self-check：per-asset SQL profile/schema 選擇、registry-only stop-tracking、manifest-backed missing SQLite table reimport、精準 `can_auto_repair` 標記、以及 opt-in real MySQL/PostgreSQL driver smoke tests 已進 UI/JSON/測試；CI 也已有受控 service-container smoke。下一步是用 disposable service tables 覆蓋 registry-backed database/table self-check，並只在 ownership 明確時擴大 guarded repair suggestion 的適用範圍。
