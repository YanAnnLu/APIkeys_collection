...
| --- | --- |
| Branch | `main` |
| 最新已驗證 checkpoint | 2026-05-24 Windows/K：`c5819a3 Split NCEI data-file resolver` 已推送，GitHub Actions run `26346356148` 成功。這是 Backend/Core Hardening 第十四個小切片：`api_launcher/adapter_plan_resolvers/ncei_search_data_file.py` 接手 NOAA/NCEI Search data-file lookup；主檔保留相容 wrapper 與主檔 `fetch_json` patch 點，NCEI Access Data guard 仍留在主檔，`adapter_plan_resolver.py` 從 2874 行降到 1404。 |
| 最新本機驗證 | temp `PYTHONPYCACHEPREFIX` `py_compile` 通過；`py -3 -m unittest tests.test_adapter_plan_resolver tests.test_dataset_download_plan -v` 共 63 tests OK；`git diff --check` OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`。完整本機 log 在 `state/logs/pre_push_smoke_20260524_072521.log`，不提交。 |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-24 目前已確認的遠端實質 checkpoint 是 `c5819a3 Split NCEI data-file resolver`；GitHub Actions run `26346356148` 成功。這是 Backend/Core Hardening 的第十四個小切片，接在 DataCite / Dataverse / CKAN / Socrata / CMR granule asset / CMR collection / STAC / ERDDAP / NCEI Search resolver 拆分後，繼續讓 `adapter_plan_resolver.py` 變成 orchestrator + 相容 wrapper。 |
...
| 本輪本機驗證 | 2026-05-24 Windows/K：Backend/Core hardening checkpoint `c5819a3` 已驗證。temp `PYTHONPYCACHEPREFIX` `py_compile` 通過；`py -3 -m unittest tests.test_adapter_plan_resolver tests.test_dataset_download_plan -v` 共 63 tests OK；`git diff --check` OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26346356148` Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260524_072521.log`，不提交。 |
