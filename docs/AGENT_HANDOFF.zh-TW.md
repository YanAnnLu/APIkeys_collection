...
| --- | --- |
| Branch | `main` |
| 最新已驗證 checkpoint | 2026-05-24 Windows/K：`2c7dc29 Split metadata guard helpers` 已推送，GitHub Actions run `26346812516` 成功。這是 Backend/Core Hardening 第十六個小切片：`api_launcher/adapter_plan_resolvers/metadata_guards.py` 接手 CMR metadata/data rel 判斷、CMR API metadata URL 判斷與 OGC Records metadata rel 判斷；主檔保留相容 wrapper，目前 `adapter_plan_resolver.py` 實測約 1430 行。 |
| 最新本機驗證 | `py -3 -m py_compile api_launcher\adapter_plan_resolver.py api_launcher\adapter_plan_resolvers\metadata_guards.py` 通過；`py -3 -m unittest tests.test_adapter_plan_resolver tests.test_dataset_download_plan -v` 共 63 tests OK；`git diff --check` OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`。完整本機 log 在 `state/logs/pre_push_smoke_20260524_075002.log`，不提交。 |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-24 目前已確認的遠端實質 checkpoint 是 `2c7dc29 Split metadata guard helpers`；GitHub Actions run `26346812516` 成功。這是 Backend/Core Hardening 的第十六個小切片，接在 DataCite / Dataverse / CKAN / Socrata / CMR granule asset / CMR collection / STAC / ERDDAP / NCEI Search / NCEI data-file / NCEI Access Data resolver 拆分後，將 CMR/OGC metadata guard 邊界也移出主檔。 |
