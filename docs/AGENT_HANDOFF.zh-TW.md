...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪功能 checkpoint 是 `c903db4 Remember UI import policy preference`：Tk UI 會記住 import policy、下載計畫面板顯示目前策略、policy helper/test 與 GTD/handoff 同步已推到 `origin/main`。push 後已用 `gh run watch 26144606542 --repo YanAnnLu/APIkeys_collection --exit-status` 確認 CI success。 |
| 上次本機驗證 | 2026-05-20 macOS/Antigravity/CloudMounter：IDE 更新後目前這個舊 Codex session 仍被 `CODEX_SANDBOX=seatbelt`、`CODEX_SANDBOX_NETWORK_DISABLED=1` 限制；`~/.codex/config.toml` 已是 `sandbox_mode = "danger-full-access"` / `approval_policy = "never"`，但本 session 沒吃到，建議關閉 IDE / reload window / 開新 Codex session 後先驗證環境變數。餘波修復：`APIkeys_collection.py` 曾被工作樹刪除，已用 HEAD 可讀內容補回；缺失 Git blob `a60de8185d6ad444079a35ede668b19b1e70c5fa` 經確認等於 `frontends/unreal/README.zh-TW.md` 工作樹 hash 後，用 `git hash-object -w` 補回。上一輪 import preference 驗證：`PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m py_compile api_launcher/import_policies.py frontends/tk/launcher_ui.py tests/test_tk_ui_preferences.py` OK；`PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m unittest tests.test_download_plan_runner tests.test_tk_ui_preferences -v`，9 tests OK；GitHub Actions run `26144606542` Windows/Ubuntu success。該 run 有 notice：`windows-latest` requests 會在 2026-06-15 前導到 `windows-2025-vs2026`；目前不是失敗，後續若 CI runner warning 變多再調整 workflow runner。本輪 guarded reimport 擴充驗證：`PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m py_compile api_launcher/database_repair.py tests/test_database_repair.py` OK；`PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m unittest tests.test_database_repair -v`，5 tests OK；sandbox 內 full test 因 seatbelt 禁止 `127.0.0.1` 測試伺服器 bind 仍需升權，升權後 `PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m unittest discover -s tests -v`，336 tests OK、skipped=1（optional numpy renderer dependency）；`git diff --check` OK。 |
| 最近新增重點 | Guarded SQLite table reimport 擴大到現有 importer 已支援的 CSV/JSON 變體：`csv.gz`、`jsonl`、`ndjson`、`geojson` 與 gzip 變體。這仍只針對 registry 裡明確納管、狀態為 missing/error、notes 有 manifest path、且目標 table 目前不存在的 SQLite table asset；不會 DROP、覆蓋既有 table 或猜測未記錄來源。上一個 checkpoint：Tk UI guided import 現在可選 keep/rename/replace，預設安全 rename，會記住使用者偏好並顯示目前匯入策略。 |
| MVP 剩餘估算 | 約 12%；bounded adapter 的 DOI/DataCite、NOAA/NCEI data-file、CMR granule data-link、常見 resource 欄位命名與 JSON-LD `@graph` 包裝風險各收掉一塊，database self-check UI 已有 profile/schema 修正、停止追蹤單一壞資產與 manifest-backed missing SQLite table reimport，且 guarded reimport 已支援 CSV/JSON/JSONL/GeoJSON 類匯入來源；plan-driven import 重跑現在有預設略過、CLI/UI safe-rename、明確 replace 三種路徑；剩下主要是擴大更多明確擁有權 repair 範圍、crawler source 類型收束、以及少量 UI polish / docs sync |
...
- 2026-05-20 Antigravity IDE 更新後，舊 Codex session 可能沒有吃到 `~/.codex/config.toml` 的 `danger-full-access` 設定，而是顯示 `CODEX_SANDBOX=seatbelt`、`CODEX_SANDBOX_NETWORK_DISABLED=1`。接力最佳做法是先關閉 IDE / reload window / 新開 Codex 對話，再第一時間跑 `env | sort | rg 'CODEX_SANDBOX|CODEX_SANDBOX_NETWORK_DISABLED'`、`curl -I --max-time 5 https://github.com`、`test -w /Users/yen-an/.codex`。若仍是 sandbox/network disabled，繼續用保守模式與逐次升權；不要嘗試用 root 硬突破。
...
2. 擴充 SQL/database self-check：per-asset SQL profile/schema 選擇、registry-only stop-tracking、以及 manifest-backed missing SQLite table reimport 已進 UI；guarded reimport 現在接受 CSV/CSV.GZ/JSON/JSONL/NDJSON/GeoJSON 類匯入來源。下一步加入真實 driver smoke 覆蓋，並只在 ownership 明確時擴大 guarded repair suggestion 的適用範圍。
