...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端實質 checkpoint 是 `77ad953 Split download plan Tk workflows`；GitHub Actions run `26335685129` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第二十輪安全解耦：把 provider/dataset version 加入下載計畫、plan key、選取項目、plan entry 組裝、Adapter plan 解析、resolved plan 匯入、Adapter 待辦面板與下載計畫匯出抽成 `PlanWorkflowMixin`；下載 queue、匯入 pipeline、Adapter resolver backend 行為不變。`launcher_ui.py` 已降到 2391 行。仍沒有搬主表格/detail 控制、crawler/source discovery UI、AI summary UI 或資料庫 repair backend。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `77ad953` 已驗證。`py_compile` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_helpers tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_provider_models tests.test_adapter_plan_resolver tests.test_ingestion_pipeline -v` 共 112 tests OK；`git diff --check` / cached check OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26335685129` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260523_224409.log`，不提交。 |
