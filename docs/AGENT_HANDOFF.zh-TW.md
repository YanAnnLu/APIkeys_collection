...
1. 實作第一個 bounded API-query adapter：可從 `adapter_review` queue 選 NOAA ERDDAP query 模板、STAC item search、或其他官方 catalog/API 到 direct plan entries 的轉換。避免一開始就硬寫大型網站流程。
