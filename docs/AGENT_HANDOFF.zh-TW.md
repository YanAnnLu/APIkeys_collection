...
1. 把 `api_launcher/library_actions.py` 的共用規則延伸到未來 agent skill。
2. 為 repair panel 增加安全的一鍵重排下載/修復建議。
3. 實作 data-store connection testing，先從 MySQL/PostgreSQL/SQLite 自檢開始。
...
目前優先任務是延伸 api_launcher/library_actions.py 的共用規則到 agent skill，並為 repair panel 補安全重排下載建議，接著做 data-store connection testing。
