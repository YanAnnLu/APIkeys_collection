...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準。最新功能 checkpoint 是 heartbeat automation：CLI 可產生 heartbeat report / plan JSON / external-agent prompt，`scripts\heartbeat_codex.cmd` 可在安全檢查通過時呼叫本機 `codex exec` 做真正的定時推進；Windows 本機互動操作優先用 `.cmd` wrappers，避免 PowerShell Execution Policy 擋 `.ps1`。上一個已確認 CI run 請以 `gh run list --limit 5` 為準。 |
