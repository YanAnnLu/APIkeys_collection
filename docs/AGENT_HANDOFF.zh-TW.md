...
| --- | --- |
| Branch | `main` |
| 最新已驗證 checkpoint | 2026-05-24 Windows/K：`065b5f4 Split manifest import CLI workflow` 已推送，GitHub Actions run `26343923135` 成功。這是 Backend/Core Hardening 第五個小切片：`api_launcher/cli_manifest_import.py` 接手 `--import-csv-manifest`、`--import-verified-csv-manifests`、`--import-json-manifest`、`--import-verified-json-manifests` 的 CLI 外殼；`core.py` 只保留四個 `*_cli(...)` 調度點，CSV/JSON importer 本體不變，`core.py` 目前約 1462 行。 |
| 最新本機驗證 | `py_compile` 通過；`py -3 -m unittest tests.test_csv_importer tests.test_json_importer tests.test_manual_import -v` 共 19 tests OK；`git diff --check` OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`。完整本機 log 在 `state/logs/pre_push_smoke_20260524_052052.log`，不提交。 |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-24 目前已確認的遠端實質 checkpoint 是 `b676907 Split manual import CLI workflow`；GitHub Actions run `26343640464` 成功。這是 Backend/Core Hardening 的第四個小切片：`api_launcher/cli_manual_import.py` 接手 `--write-local-file-manifest`、`--import-local-file` 與 `--manual-import-json` 的 CLI 外殼；`core.py` 只保留 `write_local_file_manifest_cli(...)`、`import_local_file_cli(...)`、`validate_manual_import_args(...)` 調度點。實際格式判斷仍由 `manual_import.py` 擁有，CSV/JSON SQLite 寫入仍由 importers 擁有，`core.py` 已降到 1665 行。 |
...
| 本輪本機驗證 | 2026-05-24 Windows/K：Backend/Core hardening checkpoint `b676907` 已驗證。temp `PYTHONPYCACHEPREFIX` `py_compile` 通過；`py -3 -m unittest tests.test_manual_import tests.test_csv_importer tests.test_json_importer -v` 共 19 tests OK；`git diff --check` / cached check OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26343640464` Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260524_050728.log`，不提交。 |
