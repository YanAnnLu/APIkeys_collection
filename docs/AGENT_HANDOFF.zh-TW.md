...
1. 實作第一個真正的 non-direct web/API adapter：可從 `adapter_review` queue 選一個 bounded、風險低的案例，例如 GEBCO selector 的 bounded 解析、NOAA ERDDAP query 模板，或其他官方 catalog/API 到 direct plan entries 的轉換。避免一開始就硬寫大型網站流程。
