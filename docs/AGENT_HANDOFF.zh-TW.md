...
| 上次本機驗證 | 2026-05-20 Windows/CloudMounter：database self-check stop-tracking repair UI checkpoint 使用 `.\.venv\Scripts\python.exe -B -m py_compile frontends\tk\launcher_ui.py api_launcher\repository.py` OK；`.\.venv\Scripts\python.exe -B -m unittest discover -s tests -p test_install_registry.py -v`，18 tests OK；`.\.venv\Scripts\python.exe -B -m unittest discover -s tests -p test_database_self_check.py -v`，33 tests OK；`.\.venv\Scripts\python.exe -B -m unittest discover -s tests -v`，318 tests OK、skipped=1；`git diff --check` OK（只出現 `api_launcher/repository.py` CRLF/LF 提醒，非錯誤）。注意：本機 `python` 指到 WindowsApps 佔位程式，請用 `.venv\Scripts\python.exe`；雲端磁碟寫 `__pycache__` 可能卡住，測試時可用 `-B`、設 `PYTHONDONTWRITEBYTECODE=1`，或把 pycache 指到本機暫存。前一個 DOI/DataCite checkpoint：`datacite_doi_content_url_resolver` 只查一次 DataCite DOI API，從 `contentUrl` 挑支援格式、未宣告超過 100MB 的 direct file。 |
| 最近新增重點 | Database self-check UI 現在可在 Repair / verify assets 的「資料庫」分頁中選一筆 missing/error asset，按「調整資料庫連線」修改該 asset 的 `data_store_profile_id` 與 `schema_name`，或按「停止追蹤」把單一 database/table asset 標成 `unmanaged`。兩者都只更新 launcher registry metadata 並重新自檢；不會 DROP、建立、改寫資料庫物件，也不會移動檔案。前一個重點：DOI/DataCite resource handoff 有窄範圍 lookup resolver；若 plan 是 DataCite DOI 或帶 DOI 的 OpenAlex work，且沒有既有 resources，resolver 只查一次 DataCite DOI API，從 metadata 的 `contentUrl` 裡挑支援格式、未宣告超過 100MB 的 direct file。OpenSpec CLI 已初始化 `openspec/`，Spectra GUI 已安裝到 `~/Applications/Spectra.app`，Qt Designer 已確認在 `metal_trade_312`；crawler source type dispatcher 已集中成 `SOURCE_CRAWLER_HANDLERS` mapping，portal intake 共用同一份 `SUPPORTED_DATASET_SOURCE_TYPES`；跨平台接力檢查已併入本文件 |
| MVP 剩餘估算 | 約 18-20%；bounded adapter 的 DOI/DataCite、NOAA/NCEI data-file 與 CMR granule data-link 風險各收掉一塊，database self-check UI 也已有 profile/schema 修正與停止追蹤單一壞資產；剩下主要是 adapter-owned restore/reimport repair action、crawler source 類型擴充、import policy 與少量 UI polish |
