...
1. 擴充 SQL/database self-check：MySQL/PostgreSQL schema/table introspection，以及跨引擎 DB/table asset 與 install record 的 ownership mapping。
...
目前優先任務是擴充 SQL/database self-check：MySQL/PostgreSQL schema/table introspection，以及跨引擎 DB/table asset 與 install record 的 ownership mapping；接著擴充 repair 建議到 adapter-specific datasets、agent-readable repair summaries 與更完整事件 log。
