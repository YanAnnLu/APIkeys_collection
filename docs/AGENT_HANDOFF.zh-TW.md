...
| 本次接力前新增重點 | workspace inventory CLI、`docs/WORKSPACE_LAYOUT.zh-TW.md`、handoff portal/local discovery summary、`core.py` command-detection 小拆分、crawler shared types/metadata/STAC/CKAN 小拆分 |
...
- Crawler 共用 metadata helper 已拆到 `api_launcher/crawlers/metadata.py`，STAC collection payload parser 已拆到 `api_launcher/crawlers/stac.py`，CKAN package payload parser 已拆到 `api_launcher/crawlers/ckan.py`。`dataset_sources.py` 目前仍保留 dispatcher、pagination glue 與其他尚未拆的 source parser；下一步可優先拆 ERDDAP 或 CMR。
