...
| --- | --- |
| Branch | `main` |
| 最新已驗證 checkpoint | 2026-05-25 Windows/K：`2623dc4 Treat WMS capabilities as seed entry listings` 已推送，GitHub Actions run `26396504766` 成功。這是 Dataset Seed Coverage 小切片：`ogc_wms_capabilities` 已歸入 entry-listing seed capability，避免展示報告與 crawler asset 卡片把已接 WMS crawler 的來源誤標為需要補 handler。上一個實質 checkpoint `b23f135` 則把 bounds facets 收斂到 `SOURCE_BOUND_FACETS` registry，並補上 WMS 界域 schema。 |
| 最新本機驗證 | 2026-05-25 Windows/K：WMS seed coverage 已驗證。`2623dc4` 本機驗證：crawler asset/source pattern/discovery targeted tests 74 tests OK；`py_compile` OK；`git diff --check` OK；完整 `py -B -m unittest discover -s tests` 625 tests OK、skipped=4；`scripts\pre_push_smoke_brief.cmd` 通過：625 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`。完整本機 log 在 `state/logs/pre_push_smoke_20260525_184156.log`，不提交。 |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。最新實質功能 checkpoint 是 `2623dc4 Treat WMS capabilities as seed entry listings`，CI run `26396504766` 成功；後續若只有 log / handoff 同步 commit，請視為文件狀態更新，不代表新的功能切片。 |
