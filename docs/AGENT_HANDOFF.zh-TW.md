...
| --- | --- |
| Branch | `main` |
| 最新已驗證 checkpoint | 2026-05-25 Windows/K：`cbb9deb Centralize source draft type inference` 已推送，GitHub Actions run `26395213453` 成功。這是 Discovery / Source Draft Normalization 小切片：`infer_source_type()` 已從長串 source_type if 鏈收斂為 `SOURCE_TYPE_INFERENCE_RULES` registry，只推斷已支援 crawler 的保守 URL 形狀，未知 URL 仍留在 review。上一個實質 checkpoint `c31dc41 Centralize source draft endpoint normalizers` 也已成功，endpoint 正規化已集中到 `SOURCE_ENDPOINT_NORMALIZERS` registry。最新 log-only history 同步 commit 是 `37513ec Log source draft inference registry checkpoint`，GitHub Actions run `26395322505` 也成功。 |
| 最新本機驗證 | 2026-05-25 Windows/K：第二段 source draft registry 收斂已驗證。`cbb9deb` 本機驗證：targeted discovery/source-pattern tests 74 tests OK；`py_compile` OK；`git diff --check` OK；完整 `py -B -m unittest discover -s tests` 623 tests OK、skipped=4；`scripts\pre_push_smoke_brief.cmd` 通過：623 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`。完整本機 log 在 `state/logs/pre_push_smoke_20260525_180831.log`，不提交。前一段 `c31dc41` 的 pre-push log 在 `state/logs/pre_push_smoke_20260525_175707.log`。 |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。目前遠端 HEAD 是 `37513ec Log source draft inference registry checkpoint`；最新實質功能 checkpoint 是 `cbb9deb Centralize source draft type inference`，CI run `26395213453` 成功。前一個實質 checkpoint `c31dc41 Centralize source draft endpoint normalizers` 也已記錄到 development log，CI run `26394798311` 成功。 |
