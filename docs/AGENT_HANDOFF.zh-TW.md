...
| 上次本機驗證 | 2026-05-19 macOS：`PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache conda run -n metal_trade_312 python -m unittest discover -s tests`，288 tests OK；`py_compile` Socrata crawler / dispatcher / Tk UI / core OK；`git diff --check` OK；CLI Socrata smoke `--discover-dataset-candidates --dataset-discovery-source nyc_open_data_socrata_catalog --dataset-discovery-limit 2 --upsert-dataset-candidates --db state/ci_socrata_catalog.sqlite --summary` 抓到 10 candidates、0 errors、0 warnings |
