...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端 checkpoint 是 `e68dfc8 Write provider candidates to source drafts`；GitHub Actions run `26318718206` 成功。這個 checkpoint 讓 Tk provider 候選審核面板可以把支援 crawler type/endpoint 的候選寫成 ignored local dataset discovery source 草稿；正式 catalog 仍需 local discovery dry-run audit / crawler audit / promotion guard。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：`py -3 -B -m unittest tests.test_discovery_drafts tests.test_launcher_ui tests.test_discovery tests.test_discovery_promotion -v` 共 48 tests OK；`git diff --check` OK；`scripts\pre_push_smoke_brief.cmd` 共 480 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26318718206` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260523_083602.log`，不提交。 |
| Mac 接力準備 | Mac 端先 `git pull --ff-only origin main`，再用 `PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache conda run -n metal_trade_312 python -m unittest discover -s tests` 或至少先跑 `PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache conda run -n metal_trade_312 python -m unittest tests.test_discovery_drafts tests.test_launcher_ui tests.test_discovery tests.test_discovery_promotion -v`。不要把 Windows `K:\...`、`state/`、`downloads/`、`tem/` 或 ignored local config 當成 Mac 必備狀態；若 Mac 缺本機 local seeds/source drafts，重新從 UI/CLI 產生或以 review JSON 重建。 |
