...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪最近功能 checkpoint 是 DataCite `contentUrl` resource handoff 與 DOI/OpenAlex landing/API review guard；push 後請用 `gh run list --branch main --limit 5` 與 `gh run watch ... --exit-status` 確認 CI。 |
| 上次本機驗證 | 2026-05-19 Windows/CloudMounter：DataCite/OpenAlex checkpoint 使用 `PYTHONDONTWRITEBYTECODE=1 .\.venv\Scripts\python.exe -m unittest tests.test_dataset_discovery.DatasetDiscoveryTests.test_datacite_dois_payload_becomes_research_dataset_candidate tests.test_dataset_download_plan.DatasetDownloadPlanTests.test_datacite_content_url_resource_can_resolve_to_direct_asset tests.test_dataset_download_plan.DatasetDownloadPlanTests.test_openalex_work_landing_page_requires_repository_adapter_review -v`，3 tests OK；`PYTHONDONTWRITEBYTECODE=1 .\.venv\Scripts\python.exe -m unittest tests.test_dataset_discovery tests.test_dataset_download_plan -v`，36 tests OK；`PYTHONDONTWRITEBYTECODE=1 .\.venv\Scripts\python.exe -m unittest tests.test_adapter_plan_resolver -v`，28 tests OK；`PYTHONDONTWRITEBYTECODE=1 .\.venv\Scripts\python.exe -m unittest discover -s tests -v`，306 tests OK、skipped=1；`git diff --check` OK。注意：本機 `python` 指到 WindowsApps 佔位程式，請用 `.venv\Scripts\python.exe`；雲端磁碟寫 `__pycache__` 可能卡住，測試時建議設 `PYTHONDONTWRITEBYTECODE=1` 或把 pycache 指到本機暫存。前一個 CMR checkpoint：CMR live smoke 抓到 6 candidates 並能轉成 `cmr_bounded_granule_search_resolver` 的 `/search/granules.json?collection_concept_id=...&page_size=1`。 |
| 最近新增重點 | DataCite DOI / OpenAlex work 現在會明確留在 adapter review：DOI/OpenAlex 是研究 metadata 或入口頁，不是可直接下載的檔案；DataCite metadata 若提供明確 `contentUrl`，crawler 會整理成 `resources`，讓 generic resolver 只挑安全、支援格式的檔案連結。前一個重點：`api_launcher/adapter_plan_resolver.py` 會在 CMR candidate 裡跳過 `metadata/documentation/browse/service/opendap/self/...` 這些 metadata 或服務入口，不會因為它們看似 `.json` 就提升成 direct download；真正 `rel=data` / `rel=download` / `rel=enclosure` 的直接檔案連結仍可由 generic resolver 提升，NetCDF/HDF 這類科學檔會下載為 direct asset，但 SQLite 匯入仍會標成 manual review。OGC API Records candidate 也會跳過 `self/alternate/canonical/describedby/items/related/...` metadata/navigation links；真正 `rel=data` / `rel=download` 類直接資料連結仍可由 generic resolver 提升。OpenSpec CLI 已初始化 `openspec/`，Spectra GUI 已安裝到 `~/Applications/Spectra.app`，Qt Designer 已確認在 `metal_trade_312`；crawler source type dispatcher 已集中成 `SOURCE_CRAWLER_HANDLERS` mapping，portal intake 共用同一份 `SUPPORTED_DATASET_SOURCE_TYPES`；跨平台接力檢查已併入本文件 |
...
- `api_launcher/adapter_plan_resolver.py` 是第一個 non-direct plan resolver：CLI `--resolve-adapter-plan INPUT --write-resolved-adapter-plan OUTPUT` 會掃描 CKAN/Data.gov/NCEI/CMR/STAC/Zenodo 類候選常見的 `dataset_version.metadata.resources` / `links`，只把看起來已經是 direct file URL、或 resource metadata 明確標成 CSV/JSON/ZIP 等支援格式的 URL 提升成 direct download plan entry，並重建 `target_path`、`download_eligibility`、`import_plan`。宣告超過 100 MB 的 resource 會留在 review，不自動下載。若 CKAN/Data.gov plan 只有 `package_show` URL，或只有 `package_search` URL 加 dataset id，resolver 也能只查一次單一 package metadata，再挑安全 direct resource；它不掃整個 CKAN catalog。ERDDAP candidate 若帶 `erddap_protocols`，會讀官方 `info/{dataset}/index.json`，產生最多 25 列/最小維度切片的 sample CSV plan entry，讓下載與 SQLite 匯入流程能先閉環；不要把這當成完整大量下載。Tk UI 已有 `資料庫 > 解析 Adapter 計畫`、上方 `更多 > 解析 Adapter 計畫`、以及 `Adapter 待辦 > 解析可下載 resources`。HTML、API selector、登入頁或未知格式仍留在 adapter review；這是 bounded plan rewrite，不是無頭亂爬整站。
...
- `api_launcher/adapter_plan_resolver.py` 現在也能把 NASA CMR collection candidate 轉成 bounded `page_size=1` granule metadata JSON plan entry。這只下載一筆 granule metadata，讓衛星/地球觀測來源先走通 JSON manifest/import 小閉環；不會把 `granules.json` 這類 API metadata 當成普通 direct file。CMR granule links 也新增保護：`metadata`、`documentation`、`browse`、`service`、`opendap`、`self` 等 rel 留在 review，只有明確 `data` / `download` / `enclosure` 的 direct asset link 可被提升；NetCDF/HDF 等科學檔即使可下載，匯入 SQLite 仍會是 manual review。`plans.py` 也會把 `cmr_collection` 視為 adapter-required，避免 `.json` API endpoint 被誤當成普通 direct file。
- `plans.py` 現在也會把 DataCite DOI / OpenAlex work 視為 research metadata landing/API record，先標成 `adapter_required`。白話說：DOI 像門牌，不是檔案；OpenAlex work 像研究記錄。DataCite crawler 若看到明確 `contentUrl`，會把它整理成 `resources`，讓既有 generic resolver 可以把清楚的 `.nc`、CSV、JSON 等檔案線索提升成 direct entry；DOI landing page 或 repository 頁仍留在 adapter review。
...
- `api_launcher/importers/archive_importer.py` 是第一個 bounded transform adapter：`--run-download-plan ... --import-supported-plan-results` 遇到 `import_plan.status=requires_unpack_or_adapter` 的 ZIP/TAR，會在 manifest 健康後抽出第一個 CSV/JSON/JSONL/GeoJSON member，寫衍生 manifest 到 `state/extracted/`，再接既有 CSV/JSON SQLite importer。這不是任意猜測所有壓縮包語意，只是 MVP 安全通路。
...
1. 擴充 bounded API-query adapters：CKAN package metadata lookup、Dataverse latest-version file lookup、ERDDAP sample resolver、STAC `limit=1` item metadata GeoJSON sample、NASA CMR `page_size=1` granule metadata sample、Socrata/SODA `$limit=25` sample、NOAA/NCEI Search `limit=25&offset=0` metadata sample、NOAA/NCEI Access Data 有界小查詢已先打通；CMR granule link guard 已能分辨 metadata rel 與 explicit data/download/enclosure asset link；DataCite `contentUrl` 已能交給 generic resolver，DOI/OpenAlex landing/API record 則明確留在 adapter review。下一步可補更細的 NOAA/NCEI data-file selector、CMR granule asset selector 的 auth/size/format policy、或 DOI repository/resource adapter handoff，仍要從 crawler/adapter_review handoff 進入，不要硬寫代表資料集。
