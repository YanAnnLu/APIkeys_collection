...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪最近功能 checkpoint 是 OGC Records metadata-link resolver guard；push 後請用 `gh run list --branch main --limit 5` 與 `gh run watch ... --exit-status` 確認 CI。 |
| 上次本機驗證 | 2026-05-19 macOS：OGC Records metadata-link resolver guard checkpoint 驗證 `PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache conda run -n metal_trade_312 python -m unittest tests.test_adapter_plan_resolver tests.test_dataset_download_plan tests.test_dataset_discovery`，60 tests OK；`python -m py_compile api_launcher/adapter_plan_resolver.py tests/test_adapter_plan_resolver.py` OK；`python -m unittest discover -s tests`，302 tests OK；`git diff --check` OK。前一個 OGC broker-link checkpoint：WMO live smoke 抓到 5 candidates、0 errors、0 warnings、seeded providers=54；匯出 WMO candidate plan 後，`dataset_version.download_url` 退回 HTTP(S) catalog query，不再使用 `mqtts://...` broker link 當主要 URL。前一個 CMR checkpoint：`tests.test_json_importer tests.test_adapter_plan_resolver tests.test_dataset_download_plan`，41 tests OK；CMR live smoke 抓到 6 candidates 並能轉成 `cmr_bounded_granule_search_resolver` 的 `/search/granules.json?collection_concept_id=...&page_size=1`。OpenSpec tooling checkpoint 也已驗證 `npx -y @fission-ai/openspec@latest validate --all --no-interactive`、`conda run -n metal_trade_312 pyside6-designer` 路徑存在、Spectra 安裝於 `~/Applications/Spectra.app`。 |
| 最近新增重點 | `api_launcher/adapter_plan_resolver.py` 現在會在 OGC API Records candidate 裡跳過 `self/alternate/canonical/describedby/items/related/...` 這些 metadata/navigation links，不會因為它們看似 `.geojson` 或 `application/geo+json` 就提升成 direct download；真正 `rel=data` / `rel=download` 類直接資料連結仍可由 generic resolver 提升。前一個重點：`api_launcher/crawlers/ogc_records.py` 只用 HTTP(S) `data/download/items/self` links 當 dataset primary `api_url`，WMO/WIS2 的 `mqtts://` broker links 保留在 metadata；`catalog/APIkeys_collection_catalog.*` 與 `catalog/dataset_discovery_sources.json` 已新增 WMO WIS2 source；`api_launcher/importers/json_importer.py` 支援 NASA CMR `feed.entry`。OpenSpec CLI 已初始化 `openspec/`，Spectra GUI 已安裝到 `~/Applications/Spectra.app`，Qt Designer 已確認在 `metal_trade_312`；crawler source type dispatcher 已集中成 `SOURCE_CRAWLER_HANDLERS` mapping，portal intake 共用同一份 `SUPPORTED_DATASET_SOURCE_TYPES`；跨平台接力檢查已併入本文件 |
