...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；本文件移交更新可能會是最後一個 docs-only commit。最新功能 checkpoint 是 `12419bb Cover real DB schema drift smoke`：`real-db-smoke` CI job 會在 disposable MySQL/PostgreSQL service containers 裡建立 generated smoke table，註冊成 managed table asset，並用真正 `DatabaseAssetVerifier` 驗 present/missing/schema drift 狀態；一般 Windows/Ubuntu test matrix 不需要 DB driver。GitHub Actions run `26165313576` 已確認 Windows/Ubuntu/real-db-smoke 全部 success。 |
...
- 2026-05-20 Antigravity IDE 更新後一度沒有吃到 `~/.codex/config.toml` 的 `danger-full-access` 設定，舊 session 顯示 `CODEX_SANDBOX=seatbelt`、`CODEX_SANDBOX_NETWORK_DISABLED=1`；使用者後來點到 IDE 下方權限按鈕後已恢復。最新接力前檢查：`env | sort | rg 'CODEX_SANDBOX|CODEX_SANDBOX_NETWORK_DISABLED'` 無輸出，`curl -I --max-time 5 https://github.com` 成功，`test -w /Users/yen-an/.codex` 成功，`git push` / `gh run watch` 可直接執行。下一位 Agent 仍應先跑這三個檢查；若未來又退回 sandbox/network disabled，繼續用保守模式與逐次升權，不要嘗試用 root 硬突破。
