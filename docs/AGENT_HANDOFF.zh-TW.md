...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端 checkpoint 是 `789cd05 Split repair Tk workflows`；GitHub Actions run `26333458027` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第十六輪安全解耦：把修復 / 驗證資產面板抽成 `RepairWorkflowMixin`，集中下載 manifest 掃描、database asset self-check、重新排下載、SQLite table reimport、MySQL/PostgreSQL dry-run SQL、connection metadata 編輯與 registry-only stop tracking 等高風險修復 UI 流程；修復守門規則、event log、DB repair backend 與 download queue 邊界不變。`launcher_ui.py` 已降到約 4188 行。仍沒有搬 OAuth flow 實作、下載/匯入 backend 或資料庫 repair backend。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `789cd05` 已驗證。`py_compile` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_helpers tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_provider_models -v` 共 62 tests OK；`git diff --check` / cached check OK；`.\scripts\pre_push_smoke_brief.cmd` 共 510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26333458027` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260523_210201.log`，不提交。 |
