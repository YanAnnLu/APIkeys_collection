...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端 checkpoint 是 `5221160 Split Tk UI helper boundaries`；GitHub Actions run `26323194045` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第一輪安全解耦：純 UI helper / MVP demo 訊息 / manual import 訊息 / yfinance path parser 移到 `frontends/tk/ui_helpers.py`，Provider table view-model 移到 `frontends/tk/provider_models.py`，沒有搬 dialog、thread、Tk widget lifecycle 或下載/匯入 backend 呼叫。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `5221160` 已驗證。`py_compile` 使用 temp `PYTHONPYCACHEPREFIX` 通過；`py -3 -m unittest tests.test_launcher_ui tests.test_tk_ui_helpers tests.test_tk_provider_models -v` 共 36 tests OK；`git diff --check` OK；`scripts\pre_push_smoke_brief.cmd` 共 484 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26323194045` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260523_121117.log`，不提交。 |
...
| Tk UI 實作 | `frontends/tk/launcher_ui.py`；純 helper 邊界：`frontends/tk/ui_helpers.py`；Provider table view-model：`frontends/tk/provider_models.py` |
