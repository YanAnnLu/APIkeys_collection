...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-24 目前已確認的遠端實質 checkpoint 是 `3e0d4ce Split database repair CLI workflow`；GitHub Actions run `26343188615` 成功。這是 Backend/Core Hardening 的第二個小切片：`api_launcher/cli_database_repair.py` 接手 `--reimport-missing-sqlite-table`、`--unmanage-database-asset`、`--write-database-repair-sql` 的 CLI 執行、JSON payload 與 `database_repair_completed` event；`core.py` 只保留 `run_database_repairs(self.args, self.repository, self.db_path, log_event)` 調度點。既有 repair guard、dry-run SQL、registry-only unmanage 與 `--database-repair-json` 相容行為保持不變，`core.py` 已降到 1852 行。 |
...
| 本輪本機驗證 | 2026-05-24 Windows/K：Backend/Core hardening checkpoint `3e0d4ce` 已驗證。temp `PYTHONPYCACHEPREFIX` `py_compile` 通過；`py -3 -m unittest tests.test_database_repair tests.test_database_self_check tests.test_data_store_connections -v` 共 79 tests OK；`git diff --check` / cached check OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26343188615` Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260524_044525.log`，不提交。 |
...
10. Backend/Core Hardening 已完成 yfinance CLI compatibility slice 與 database-repair CLI slice；下一刀仍只選一個可測邊界：download/import CLI 群組，或 `adapter_plan_resolver.py` 的 source-specific resolver 拆分。DB migration runner、真實 subcommands、crawler dynamic discovery、credential catalog 化仍需 OpenSpec 先定義相容、rollback 與 CI gate。
