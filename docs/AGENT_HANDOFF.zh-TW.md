...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪功能 checkpoint 是 archive transform / source provenance：registry 會正式保存 compound source format，下載檔案與 curated table asset 會記錄實際 payload format；ZIP/TAR bounded transform 可抽 CSV/CSV.GZ/JSON/JSON.GZ/JSONL/NDJSON/GeoJSON 類 member 後接 SQLite importer。push 後請用 `gh run list --repo YanAnnLu/APIkeys_collection --branch main --limit 5` 與 `gh run watch RUN_ID --repo YanAnnLu/APIkeys_collection --exit-status` 確認 CI。 |
| 上次本機驗證 | 2026-05-20 macOS/Antigravity/CloudMounter：IDE 更新後目前這個舊 Codex session 仍被 `CODEX_SANDBOX=seatbelt`、`CODEX_SANDBOX_NETWORK_DISABLED=1` 限制；`~/.codex/config.toml` 已是 `sandbox_mode = "danger-full-access"` / `approval_policy = "never"`，但本 session 沒吃到，建議關閉 IDE / reload window / 開新 Codex session 後先驗證環境變數。餘波修復：`APIkeys_collection.py` 曾被工作樹刪除，已用 HEAD 可讀內容補回；缺失 Git blob `a60de8185d6ad444079a35ede668b19b1e70c5fa` 經確認等於 `frontends/unreal/README.zh-TW.md` 工作樹 hash 後，用 `git hash-object -w` 補回。本輪 archive transform 驗證：`PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m py_compile api_launcher/importers/archive_importer.py api_launcher/downloads/plan_runner.py tests/test_download_plan_runner.py` OK；`PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m unittest tests.test_download_plan_runner -v`，8 tests OK；sandbox 內 full test 因 seatbelt 禁止 `127.0.0.1` 測試伺服器 bind 仍需升權，升權後 `PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m unittest discover -s tests -v`，339 tests OK、skipped=1（optional numpy renderer dependency）；`git diff --check` OK。上一個 source provenance checkpoint 已由 GitHub Actions run `26145565066` Windows/Ubuntu success 驗證；該 run 仍有 `windows-latest` 轉向 `windows-2025-vs2026` notice，不是失敗。 |
| 最近新增重點 | Archive bounded transform 現在支援 ZIP/TAR 內的 `json.gz`、`jsonl.gz`、`ndjson`、`ndjson.gz`、`geojson.gz`，會寫衍生 manifest 並接既有 JSON importer；regression 已覆蓋 zip 內 `sample.ndjson.gz` 匯入 SQLite。Registry/source provenance 也正式接受並保存 compound source format，例如 `csv.gz`、`jsonl`、`ndjson`、`geojson`、`geojson.gz`、`zip`、`tar.gz`；下載檔案 asset 與 CSV/JSON/GeoJSON 匯入後的 curated table asset 會記錄實際 payload format。Guarded SQLite table reimport 也會用同一組支援格式，且錯誤/UI 文案會列出支援清單。這仍不會 DROP、覆蓋既有 table 或猜測未記錄來源。 |
| MVP 剩餘估算 | 約 11-12%；bounded adapter 的 DOI/DataCite、NOAA/NCEI data-file、CMR granule data-link、常見 resource 欄位命名與 JSON-LD `@graph` 包裝風險各收掉一塊，archive transform 已可處理 ZIP/TAR 內常見 CSV/JSON/GeoJSON 變體，database self-check UI 已有 profile/schema 修正、停止追蹤單一壞資產與 manifest-backed missing SQLite table reimport，且 guarded reimport 已支援 CSV/JSON/JSONL/GeoJSON 類匯入來源；plan-driven import 重跑現在有預設略過、CLI/UI safe-rename、明確 replace 三種路徑；剩下主要是擴大更多明確擁有權 repair 範圍、crawler source 類型收束、以及少量 UI polish / docs sync |
