...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準。最新功能 checkpoint 是 handoff report 進度上下文與 repair observability：`--handoff-report` 會列出 open GTD focus / latest verification timestamps，`--verify-downloads` / `--verify-downloads-json` 會寫入 `download_manifest_verification_completed` structured event；上一個已確認 CI run `26171027033` Windows/Ubuntu/real-db-smoke 全部 success。 |
...
| 本輪本機驗證 | 2026-05-20 Windows/K: 補 `download_manifest_verification_completed` event log 後，`tests.test_repair tests.test_event_log tests.test_handoff -v` 共 11 tests OK；`py -B -m unittest discover -s tests` 共 350 tests OK、skipped=4；`py_compile` OK；CLI smoke `--verify-downloads-json --downloads-root state\empty_repair_smoke` 輸出純 JSON，`--show-logs 3` 可看到 `download_repair:download_manifest_verification_completed`；`git diff --check` OK。 |
| 最近新增重點 | Archive bounded transform 現在支援 ZIP/TAR 內的 `csv.gz`、`json.gz`、`jsonl.gz`、`ndjson`、`ndjson.gz`、`geojson.gz`，會寫衍生 manifest 並接既有 CSV/JSON importer；regression 已覆蓋 ZIP 內 `sample.ndjson.gz` 與 TAR.GZ 內 `sample.geojson.gz` 匯入 SQLite。Registry/source provenance 也正式接受並保存 compound source format，例如 `csv.gz`、`jsonl`、`ndjson`、`geojson`、`geojson.gz`、`zip`、`tar.gz`；下載檔案 asset 與 CSV/JSON/GeoJSON 匯入後的 curated table asset 會記錄實際 payload format。Guarded SQLite table reimport 也會用同一組支援格式，且錯誤/UI 文案會列出支援清單；現在 CLI/agent 可用 `--reimport-missing-sqlite-table ASSET_ID --database-repair-json` 走同一個安全 guard，也可用 `--unmanage-database-asset ASSET_ID --database-repair-json` 做 registry-only 停止追蹤；兩者成功後都會寫入 `database_repair_completed` structured event log。下載 manifest verification 現在也會寫 `download_manifest_verification_completed`，讓 handoff report / `--show-logs` 能看到最近一次檔案健康掃描。本輪讓 opt-in real MySQL/PostgreSQL smoke tests 進一步覆蓋 registry-backed table self-check：`APIKEYS_REAL_DB_SMOKE_ALLOW_WRITE=1` 時才會建立/清掉 `apikeys_ci_registry_smoke_*` 測試表，確認 managed table asset 的 present/missing 狀態，並 ALTER 同一張 generated table 觸發 schema fingerprint drift error。`docs/SETUP.zh-TW.md` 已補本機 disposable Docker/env-var 指南；一般本機/Windows/Ubuntu 全量測試仍預設不連線、不 DROP、不覆蓋既有 table、也不猜測未記錄來源。 |
| MVP 剩餘估算 | 約 6.3% 到 6.8%；bounded adapter 的 DOI/DataCite、NOAA/NCEI data-file、CMR granule data-link、常見 resource 欄位命名與 JSON-LD `@graph` 包裝風險各收掉一塊，archive transform 已可處理 ZIP/TAR 內常見 CSV/JSON/GeoJSON 變體，database self-check UI 已有 profile/schema 修正、停止追蹤單一壞資產與 manifest-backed missing SQLite table reimport，且 guarded reimport 已支援 CSV/JSON/JSONL/GeoJSON 類匯入來源；database repair CLI 與 download manifest verification 現在都會寫 structured event log；real MySQL/PostgreSQL smoke tests 已接進受控 CI service containers 並覆蓋 registry-backed present/missing/schema drift table self-check；plan-driven import 重跑現在有預設略過、CLI/UI safe-rename、明確 replace 三種路徑；剩下主要是擴大更多明確擁有權 repair 範圍、crawler source 類型收束、少量 UI polish / docs sync |
...
- `--verify-downloads-json` 已提供下載檔驗證的 agent-readable JSON：包含 summary、issues、repair suggestion、以及 HTTP(S) manifest 可安全重排下載的 plan entry。若要指定掃描資料夾，可搭配 `--downloads-root PATH`。每次 `--verify-downloads` / `--verify-downloads-json` 掃描後也會寫入 `download_manifest_verification_completed` structured event，記錄 checked/issue/requeue counts 與最多 20 筆 issue preview，方便 `--show-logs` 和 handoff report 回溯最近一次檔案健康檢查。
