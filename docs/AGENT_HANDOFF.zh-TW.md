...
| 上次本機驗證 | 2026-05-18：Windows `py -m unittest discover -s tests`，274 tests OK；`py -m py_compile ...` OK；CI 同款 `py APIkeys_collection.py --init-db --seed --manifest-health --db state\ci.sqlite --summary` OK；`git diff --check` OK |
| 本次接力前新增重點 | workspace inventory CLI、`docs/WORKSPACE_LAYOUT.zh-TW.md`、handoff portal/local discovery summary、`core.py` command-detection 小拆分、crawler shared types/metadata/STAC/CKAN/ERDDAP/CMR/GBIF/Dataverse/Zenodo/HTML index/NCEI 小拆分 |
...
- Crawler 共用 metadata helper 已拆到 `api_launcher/crawlers/metadata.py`，HTTP fetch/JSON/URL helper 已拆到 `api_launcher/crawlers/fetch.py`，共用 full-crawl page cap 與候選去重 append helper 已拆到 `api_launcher/crawlers/pagination.py`，STAC collection payload parser 與 STAC pagination flow 已拆到 `api_launcher/crawlers/stac.py`，CKAN package query URL builder/payload parser/pagination flow 已拆到 `api_launcher/crawlers/ckan.py`，ERDDAP `allDatasets` payload parser 已拆到 `api_launcher/crawlers/erddap.py`，NASA CMR collection query URL builder/payload parser/pagination flow 已拆到 `api_launcher/crawlers/cmr.py`，GBIF dataset search query URL builder/payload parser/pagination flow 已拆到 `api_launcher/crawlers/gbif.py`，Dataverse search query URL builder/payload parser/pagination flow 已拆到 `api_launcher/crawlers/dataverse.py`，Zenodo records query URL builder/payload parser/pagination flow 已拆到 `api_launcher/crawlers/zenodo.py`，HTML file index parser 已拆到 `api_launcher/crawlers/html_index.py`，NCEI search query URL builder/payload parser/pagination flow 已拆到 `api_launcher/crawlers/ncei.py`。`dataset_sources.py` 目前主要保留 dispatcher 與 non-full query 的 fetch/parse glue。
...
- 第 1 項目前已調整為「善用 crawler 發現 provider/source 與 dataset candidates」，不要把每個代表資料集都硬寫成 Python adapter。`catalog/APIkeys_collection_catalog.json` 目前有 48 個 provider seed，新增方向包含 NOAA GOES-R on AWS、NOAA NOMADS、Marine Regions、GADM、OpenStreetMap Overpass、U.S. Census TIGERweb、EMODnet ERDDAP、Harvard Dataverse、Zenodo、Canada/UK/Australia/HDX CKAN。`catalog/dataset_discovery_sources.json` 描述可爬的資料目錄，目前 17 個 source；`api_launcher/crawlers/orchestrator.py` 統一調度 source crawlers，並行執行、去重、收斂 per-source error/warning；`api_launcher/crawlers/fetch.py` 已放共用 HTTP fetch/JSON/URL helper；`api_launcher/crawlers/pagination.py` 已放共用 full-crawl page cap 與候選去重 append helper；`api_launcher/crawlers/stac.py` 已放 STAC collection payload parser 與 STAC pagination flow；`api_launcher/crawlers/ckan.py` 已放 CKAN package query URL builder、payload parser 與 CKAN pagination flow；`api_launcher/crawlers/erddap.py` 已放 ERDDAP `allDatasets` payload parser；`api_launcher/crawlers/cmr.py` 已放 NASA CMR collection query URL builder、payload parser 與 CMR pagination flow；`api_launcher/crawlers/gbif.py` 已放 GBIF dataset search query URL builder、payload parser 與 GBIF pagination flow；`api_launcher/crawlers/dataverse.py` 已放 Dataverse search query URL builder、payload parser 與 Dataverse pagination flow；`api_launcher/crawlers/zenodo.py` 已放 Zenodo records query URL builder、payload parser、Zenodo pagination flow 與檔案摘要 helper；`api_launcher/crawlers/html_index.py` 已放 HTML file index parser；`api_launcher/crawlers/ncei.py` 已放 NOAA/NCEI Search query URL builder、payload parser 與 NCEI pagination flow；`api_launcher/crawlers/dataset_sources.py` 目前主要保留 dispatcher 與 non-full query 的 fetch/parse glue。Crawler 審核不能只看「沒報錯」：0 筆候選、低於 `min_expected_candidates`、只抓到全域重複候選、payload shape 不符、候選缺少 evidence/source url 都要提示或失敗。未來新增供應商時，優先新增/配置 crawler，由 orchestrator 調度；不要讓特殊網頁邏輯散進 UI 或 core。AIS 與衛星雲圖是代表測試案例：AIS 應由 MarineCadastre index 發現 shards，衛星雲圖應由 NOAA/NCEI/GOES-R/Earth Engine/STAC 類 catalog 發現 raster/grid 候選。
...
- `api_launcher/adapter_plan_resolver.py` 是第一個 non-direct plan resolver：CLI `--resolve-adapter-plan INPUT --write-resolved-adapter-plan OUTPUT` 會掃描 CKAN/Data.gov/NCEI/CMR/STAC/Zenodo 類候選常見的 `dataset_version.metadata.resources` / `links`，只把看起來已經是 direct file URL、或 resource metadata 明確標成 CSV/JSON/ZIP 等支援格式的 URL 提升成 direct download plan entry，並重建 `target_path`、`download_eligibility`、`import_plan`。宣告超過 100 MB 的 resource 會留在 review，不自動下載。它也有第一個 bounded API-query resolver：ERDDAP candidate 若帶 `erddap_protocols`，會讀官方 `info/{dataset}/index.json`，產生最多 25 列/最小維度切片的 sample CSV plan entry，讓下載與 SQLite 匯入流程能先閉環；不要把這當成完整大量下載。Tk UI 已有 `資料庫 > 解析 Adapter 計畫`、上方 `更多 > 解析 Adapter 計畫`、以及 `Adapter 待辦 > 解析可下載 resources`。HTML、API selector、登入頁或未知格式仍留在 adapter review；這是 bounded plan rewrite，不是無頭亂爬整站。
...
1. 擴充 bounded API-query adapters：ERDDAP sample resolver 已先打通一條小樣本 CSV 閉環，STAC collection 也已有 `limit=1` item metadata GeoJSON sample；下一步可補 OGC API Records、NOAA/NCEI query 模板，仍要從 crawler/adapter_review handoff 進入，不要硬寫代表資料集。
...
3. 繼續擴充 crawler source 類型，但要維持設定檔驅動；Dataverse/Zenodo 已有 metadata parser 與 pagination flow，下一批可評估 OGC API Records、Socrata、OpenAlex/DataCite 類 metadata 來源。
