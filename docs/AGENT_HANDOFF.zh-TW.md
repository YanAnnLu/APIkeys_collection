...
- 本輪不改實際下載、匯入、adapter resolver 執行行為，只增加可序列化 contract。已驗證：`py -B -m py_compile api_launcher\content_registry.py api_launcher\plans.py tests\test_content_registry.py` OK；`tests.test_content_registry` 12 tests OK；`tests.test_adapter_plan_resolver` 54 tests OK；`tests.test_web_preview tests.test_crawler_asset_download tests.test_crawler_assets` 81 tests OK；`git diff --check` OK（僅 docs CRLF/LF warning）；docs mojibake scan OK；`.\scripts\pre_push_smoke_brief.cmd` 通過，796 tests / 4 skipped，MVP demo smoke `download_import_completed` / `row_count=3`，log：`state\logs\pre_push_smoke_20260528_003747.log`。待 commit / push / CI。
