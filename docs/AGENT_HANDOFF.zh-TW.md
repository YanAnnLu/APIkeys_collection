...
- 已驗證：`PYTHONDONTWRITEBYTECODE=1 py -3 -B -m unittest tests.test_tk_ui_helpers tests.test_crawler_asset_download -v` 通過 33 tests；`PYTHONDONTWRITEBYTECODE=1 py -3 -B -m unittest tests.test_tk_dialogs tests.test_web_preview tests.test_crawler_asset_download -v` 通過 195 tests；docs mojibake scan OK；完整 smoke `state\logs\pre_push_smoke_20260531_010544.log` 通過，1008 tests / 4 skipped，MVP demo `download_import_completed` / `row_count=3`；`git diff --check` OK。
