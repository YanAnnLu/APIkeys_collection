...
- Tk UI 的 `Tools > Repair / verify manifests` 現在會開啟 repair panel，列出每個 download manifest 的健康狀態與路徑。
- Repair panel 現在會顯示安全修復建議；對有 HTTP(S) `source_url` 和 `provider_id` 的 missing/size/checksum manifest，可用 `Requeue selected` 透過 staging 重新排下載。
...
- SQL/database self-check 已有第一版：`--self-check-databases` 會用 registry asset verifier 檢查 managed database assets；SQLite asset 會依 `source_uri`/path 做 read-only 檢查、計算 table/column schema fingerprint、偵測已登記 fingerprint 的 drift，並回寫 asset/provider 狀態。

## 下一步優先事項

1. 擴充 SQL/database self-check：MySQL/PostgreSQL schema/table introspection，以及 DB asset/table/install record 的 ownership mapping。
2. 擴充 repair 建議到 adapter-specific datasets、agent-readable repair summaries 與更完整事件 log。
3. 擴充 `.codex/skills/apikeys-collection-launcher`，加入 repair/download/database self-check 操作流程。
...
目前優先任務是擴充 SQL/database self-check：MySQL/PostgreSQL schema/table introspection，以及 DB asset/table/install record 的 ownership mapping；接著擴充 repair 建議到 adapter-specific datasets、agent-readable repair summaries 與更完整事件 log。
