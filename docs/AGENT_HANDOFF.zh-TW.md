...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端 checkpoint 是 `784b1af Split OAuth Tk workflows`；GitHub Actions run `26333703204` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第十七輪安全解耦：把 OAuth Client ID 設定、Google browser login、本機 callback server、device-code/QR login、API key session 與 OAuth token 儲存/啟用流程抽成 `OAuthWorkflowMixin`；Google OAuth 仍維持中期/開發者功能邊界，沒有新增 live credential probing、背景登入或產品級 OAuth broker。`launcher_ui.py` 已降到約 3495 行。仍沒有搬下載/匯入 backend 或資料庫 repair backend。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `784b1af` 已驗證。`py_compile` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_helpers tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_provider_models tests.test_oauth_device -v` 共 72 tests OK；`git diff --check` / cached check OK；`.\scripts\pre_push_smoke_brief.cmd` 共 510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26333703204` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260523_211342.log`，不提交。 |
