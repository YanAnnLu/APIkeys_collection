...
| 上次本機驗證 | 2026-05-19：Windows `py -m unittest discover -s tests`，274 tests OK；`py -m py_compile ...` OK；CI 同款 `py APIkeys_collection.py --init-db --seed --manifest-health --db state\ci.sqlite --summary` OK；`git diff --check` OK |
| 本次接力前新增重點 | workspace inventory CLI、`docs/WORKSPACE_LAYOUT.zh-TW.md`、handoff portal/local discovery summary、`core.py` command-detection 小拆分、crawler shared types/metadata/STAC/CKAN/ERDDAP/CMR/GBIF/Dataverse/Zenodo/HTML index/NCEI 小拆分，source-level fetch/parse flow 已搬回各 source 模組 |
