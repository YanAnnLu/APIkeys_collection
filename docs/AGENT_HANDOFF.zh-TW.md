...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪最近功能 checkpoint 是 NOAA/NCEI Search data-file selector；push 後請用 `gh run list --branch main --limit 5` 與 `gh run watch ... --exit-status` 確認 CI。 |
| 上次本機驗證 | 2026-05-19 Windows/CloudMounter：NOAA/NCEI data-file selector checkpoint 使用 `PYTHONDONTWRITEBYTECODE=1 .\.venv\Scripts\python.exe -m unittest tests.test_adapter_plan_resolver -v`，32 tests OK；`PYTHONDONTWRITEBYTECODE=1 .\.venv\Scripts\python.exe -m unittest tests.test_dataset_download_plan tests.test_download_eligibility -v`，18 tests OK；`PYTHONDONTWRITEBYTECODE=1 .\.venv\Scripts\python.exe -m unittest discover -s tests -v`，310 tests OK、skipped=1；`git diff --check` OK。注意：本機 `python` 指到 WindowsApps 佔位程式，請用 `.venv\Scripts\python.exe`；雲端磁碟寫 `__pycache__` 可能卡住，測試時建議設 `PYTHONDONTWRITEBYTECODE=1` 或把 pycache 指到本機暫存。前一個 CMR checkpoint：CMR live smoke 抓到 6 candidates 並能轉成 `cmr_bounded_granule_search_resolver` 的 `/search/granules.json?collection_concept_id=...&page_size=1`。 |
| 最近新增重點 | NOAA/NCEI Search data-file selector 現在有一個窄範圍 direct file handoff：若 plan 已經是 `/access/services/search/v1/data`，且 query 有 dataset 加站點/空間條件，resolver 只查 1 筆 NCEI metadata；若結果有 `/data/...` direct file path、格式可辨識、且 `fileSize` 未超過 100MB，才提升成 direct download。沒有站點/空間條件、或檔案過大時，仍回到 bounded Search JSON metadata sample 或 adapter review。前一個重點：CMR granule metadata 也有窄範圍 asset selector；metadata/documentation/browse/service/opendap/self 仍留在 review。OpenSpec CLI 已初始化 `openspec/`，Spectra GUI 已安裝到 `~/Applications/Spectra.app`，Qt Designer 已確認在 `metal_trade_312`；crawler source type dispatcher 已集中成 `SOURCE_CRAWLER_HANDLERS` mapping，portal intake 共用同一份 `SUPPORTED_DATASET_SOURCE_TYPES`；跨平台接力檢查已併入本文件 |
| MVP 剩餘估算 | 約 20-23%；bounded adapter 的 NOAA/NCEI data-file 與 CMR granule data-link 風險各收掉一塊，剩下主要是 DOI repository/resource handoff、database self-check UI/repair action、crawler source 類型擴充、import policy 與少量 UI polish |
