...
1. 實作第一個真正的 non-direct / transform adapter：可從 `adapter_review` queue 選一個 bounded、風險低的案例，例如 ZIP/TAR 解包、CSV.ZST 外部工具偵測，或 GEBCO selector 的 bounded 解析。避免一開始就硬寫大型網站流程。
