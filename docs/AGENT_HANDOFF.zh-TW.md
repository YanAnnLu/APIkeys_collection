...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端 checkpoint 是 `20df3d8 Split Tk startup and desktop helpers`；GitHub Actions run `26323548216` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第二輪安全解耦：Tk 啟動失敗修復提示與 `TclError` suppressor 移到 `frontends/tk/startup_helpers.py`，跨平台檔案管理器定位 helper 移到 `frontends/tk/desktop_integration.py`，並補桌面整合 helper regression；仍沒有搬 dialog、thread、Tk widget lifecycle 或下載/匯入 backend 呼叫。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `20df3d8` 已驗證。`py_compile` 使用 temp `PYTHONPYCACHEPREFIX` 通過；`py -3 -m unittest tests.test_launcher_ui tests.test_tk_desktop_integration tests.test_tk_ui_helpers tests.test_tk_provider_models -v` 共 39 tests OK；`git diff --check` OK；`.\scripts\pre_push_smoke_brief.cmd` 共 487 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26323548216` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260523_122939.log`，不提交。 |
...
| Tk UI 實作 | `frontends/tk/launcher_ui.py`；純 helper 邊界：`frontends/tk/ui_helpers.py`；Provider table view-model：`frontends/tk/provider_models.py`；啟動/Tcl helper：`frontends/tk/startup_helpers.py`；桌面檔案管理器 helper：`frontends/tk/desktop_integration.py` |
