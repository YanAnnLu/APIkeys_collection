...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-24 目前已確認的遠端實質 checkpoint 是 `50724d2 Split Tk detail panel workflows`；GitHub Actions run `26337627361` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第二十六輪安全解耦：把右側細節抽屜建置、provider detail 更新、描述 fallback 與 preview/AI summary 文字框 setter 抽成 `DetailPanelWorkflowMixin`。`launcher_ui.py` 已降到 947 行。仍沒有搬 row action/crawl/install/uninstall、修復面板 backend 或 database repair backend；這些留給下一輪較高風險切片。 |
