...
- Tk UI 現在有 `整合 > Google 帳號登入（瀏覽器）` 直達入口，也保留 `整合 > 進階：Google QR / 裝置碼登入`。若缺 Google OAuth Client ID，UI 會先開 `Google 登入前置設定` 導引頁，說明這不是 Gmail / 密碼 / API key，再讓使用者貼上 Client ID 並寫入 `config/launcher_integrations.local.json`；`api_launcher/oauth_device.py` 仍保留環境變數 fallback。
