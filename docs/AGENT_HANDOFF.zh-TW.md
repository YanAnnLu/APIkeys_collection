...
- Web Preview 卡片牆現在會在本次 session 建立下載計畫後顯示 `plan_outcome` 徽章：文字吃 `short_label`，色調吃 `display_tone`，若 `content_review.has_review` 也會顯示內容格式待辦徽章。這仍是 UIUX preview 的 session-local 狀態；跨 session 持久化應回到 event log / asset profile / backend service，不要直接把狀態塞進 JS localStorage。
