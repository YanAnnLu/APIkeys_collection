...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪最近功能 checkpoint 是 CMR granule asset link selector；push 後請用 `gh run list --branch main --limit 5` 與 `gh run watch ... --exit-status` 確認 CI。 |
| 上次本機驗證 | 2026-05-19 Windows/CloudMounter：CMR granule asset selector checkpoint 使用 `PYTHONDONTWRITEBYTECODE=1 .\.venv\Scripts\python.exe -m unittest tests.test_adapter_plan_resolver -v`，30 tests OK；`PYTHONDONTWRITEBYTECODE=1 .\.venv\Scripts\python.exe -m unittest tests.test_dataset_download_plan tests.test_download_eligibility -v`，18 tests OK；`PYTHONDONTWRITEBYTECODE=1 .\.venv\Scripts\python.exe -m unittest discover -s tests -v`，308 tests OK、skipped=1；`git diff --check` OK。注意：本機 `python` 指到 WindowsApps 佔位程式，請用 `.venv\Scripts\python.exe`；雲端磁碟寫 `__pycache__` 可能卡住，測試時建議設 `PYTHONDONTWRITEBYTECODE=1` 或把 pycache 指到本機暫存。前一個 CMR checkpoint：CMR live smoke 抓到 6 candidates 並能轉成 `cmr_bounded_granule_search_resolver` 的 `/search/granules.json?collection_concept_id=...&page_size=1`。 |
| 最近新增重點 | CMR granule metadata 現在有一個窄範圍 asset selector：若 plan 已經是單筆 CMR granule metadata，resolver 只查一次 CMR concept/granules JSON，從 `links` 裡挑明確 `data` / `download` / `enclosure` rel、格式可辨識、未宣告超過 100MB 的檔案連結；metadata/documentation/browse/service/opendap/self 仍留在 review。前一個重點：DataCite DOI / OpenAlex work 會明確留在 adapter review；DataCite `contentUrl` 會整理成 `resources`，讓 generic resolver 只挑安全、支援格式的檔案連結。OpenSpec CLI 已初始化 `openspec/`，Spectra GUI 已安裝到 `~/Applications/Spectra.app`，Qt Designer 已確認在 `metal_trade_312`；crawler source type dispatcher 已集中成 `SOURCE_CRAWLER_HANDLERS` mapping，portal intake 共用同一份 `SUPPORTED_DATASET_SOURCE_TYPES`；跨平台接力檢查已併入本文件 |
| MVP 剩餘估算 | 約 21-24%；bounded adapter 的 CMR granule data-link 風險又收掉一塊，剩下主要是 NOAA/NCEI data-file selector、DOI repository/resource handoff、database self-check UI/repair action、crawler source 類型擴充、import policy 與少量 UI polish |
