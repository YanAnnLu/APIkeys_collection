...
1. 擴充 repair 建議到 adapter-specific datasets，並把 download/database JSON repair payload 接到更完整事件 log 與 UI guided repair flows；可以先從讀取 plan entry 的 `adapter_review` 開始。
