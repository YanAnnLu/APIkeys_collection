...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端 checkpoint 是 `5cf1c2a Split Tk dialog classes`；GitHub Actions run `26324695216` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第五輪安全解耦：把有明確生命週期和狀態邊界的 `ProviderEditorDialog`、`DatabaseClientSettingsDialog` 搬到 `frontends/tk/dialogs.py`，主檔只保留開窗與消費結果的呼叫點；仍沒有搬下載 queue、背景 thread、repair/import backend 或一般 Toplevel workflow。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `5cf1c2a` 已驗證。`py_compile` 使用 temp `PYTHONPYCACHEPREFIX` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_ui_helpers tests.test_tk_provider_models -v` 共 53 tests OK；`git diff --check` OK；`.\scripts\pre_push_smoke_brief.cmd` 共 501 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26324695216` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260523_132912.log`，不提交。 |
...
| Tk UI 實作 | `frontends/tk/launcher_ui.py`；可獨立封裝的 dialog class：`frontends/tk/dialogs.py`；UI 設定/常數：`frontends/tk/ui_config.py`；狀態/修復文案：`frontends/tk/ui_labels.py`；純 helper 邊界：`frontends/tk/ui_helpers.py`；Provider table view-model：`frontends/tk/provider_models.py`；啟動/Tcl helper：`frontends/tk/startup_helpers.py`；桌面檔案管理器 helper：`frontends/tk/desktop_integration.py` |
