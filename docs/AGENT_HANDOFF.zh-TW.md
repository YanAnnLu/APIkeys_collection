...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準。最新功能 checkpoint 是 repair observability：CLI/Tk manifest verification 共用 `download_manifest_verification_completed` event schema，Tk repair requeue 會寫 `download_repair_requeue_requested`，`--handoff-report` 會列出 open GTD focus / latest verification timestamps / latest download requeue outcome。上一個已確認 CI run 請以 `gh run list --limit 5` 為準。 |
...
| 本輪本機驗證 | 2026-05-20 Windows/K: repair observability 追加 UI requeue event 後，`tests.test_repair tests.test_event_log tests.test_handoff -v` 共 13 tests OK；`py -B -m unittest discover -s tests` 共 352 tests OK、skipped=4；`py_compile` OK；CLI smoke `--verify-downloads-json --downloads-root state\empty_repair_smoke` 輸出純 JSON，`--show-logs 5` 可看到 `download_repair:download_manifest_verification_completed`；`--handoff-report state\handoff_requeue_smoke.md` 可看到 latest download requeue 欄位；`git diff --check` OK。 |
