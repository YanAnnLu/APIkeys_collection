...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-24 目前已確認的遠端實質 checkpoint 是 `78792d3 Split Tk window layout workflows`；GitHub Actions run `26341730759` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第二十七輪安全解耦：把主視窗樣式、選單列與第一層 layout 組裝抽成 `WindowLayoutWorkflowMixin`。`launcher_ui.py` 已降到 709 行，並已補回 `PROJECT_ROOT` 相容匯出。仍沒有搬 row action/crawl/install/uninstall 或最後 backend dispatch 控制路徑；這些留給下一輪較高風險切片。 |
...
| 本輪本機驗證 | 2026-05-24 Windows/K：UI 解耦 checkpoint `78792d3` 已驗證。temp `PYTHONPYCACHEPREFIX` `py_compile` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_helpers tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_provider_models -v` 共 62 tests OK；`git diff --check` / cached check OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26341730759` Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260524_033308.log`，不提交。 |
