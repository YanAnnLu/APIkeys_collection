...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端實質 checkpoint 是 `ae8ba54 Split AI summary Tk workflows`；GitHub Actions run `26336316057` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第二十二輪安全解耦：把啟動時本機 AI API key 載入、AI profile label/login 狀態、credential 檢查、provider summary 背景 worker 與 repository 回寫抽成 `AiSummaryWorkflowMixin`；OAuth workflow/dialog 仍透過同一組 profile 狀態 helper，startup 不觸發瀏覽器登入或 OAuth 視窗。`launcher_ui.py` 已降到 1736 行。仍沒有搬主表格/detail 控制或資料庫 repair backend。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `ae8ba54` 已驗證。`py_compile` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_helpers tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_provider_models -v` 共 62 tests OK；`git diff --check` / cached check OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26336316057` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整通過 log 在 `state/logs/pre_push_smoke_20260523_231338.log`，不提交。 |
