...
1. 擴充 dataset crawler：從 provider/source discovery 進一步產生 dataset candidates，優先讓 NOAA/NCEI、ERDDAP、MarineCadastre AIS、GOES-R/Earth Engine 這些來源由設定檔驅動，而不是硬編碼 adapter。
2. 做 dataset candidate review/import UI，讓使用者能審核 crawler 找到的資料集，再排入下載/安裝計畫。
...
目前優先任務是把第 1 項改成 crawler-first：provider/source discovery 找供應商與入口，dataset discovery sources 找資料集候選，adapter 只在 crawler 候選需要 bounded query/auth/transform/import 時才寫。請優先看 `catalog/dataset_discovery_sources.json`、`api_launcher/dataset_discovery.py`、`api_launcher/cli_dataset_discovery.py`。AIS 與衛星雲圖是代表測試案例，但不要再把每個資料集硬寫成 Python 類別。
