...
- SQL/database self-check 已有第一版：`--self-check-databases` 會用 registry asset verifier 檢查 managed database assets；SQLite asset 會依 `source_uri`/path 做 read-only 檢查並回寫 asset/provider 狀態。

## 下一步優先事項

1. 擴充 SQL/database self-check：MySQL/PostgreSQL schema/table introspection、table/schema fingerprints、drift/missing database summaries。
...
目前優先任務是擴充 SQL/database self-check：MySQL/PostgreSQL schema/table introspection、table/schema fingerprints、drift/missing database summaries；接著擴充 repair 建議到 adapter-specific datasets、agent-readable repair summaries 與更完整事件 log。
