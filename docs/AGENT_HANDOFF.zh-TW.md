...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-24 目前已確認的遠端實質 checkpoint 是 `dc7523d Split yfinance CLI workflow`；GitHub Actions run `26342897778` 成功。這是 Backend/Core Hardening 的第一個小切片：`api_launcher/cli_yfinance.py` 接手 yfinance argparse flags、command-active 判斷與 CLI 執行流程；`core.py` 只保留 `add_yfinance_args(parser)` 與 `run_yfinance_cli(args)` 兩個調度點，不再直接 import yfinance 常數或 writer。既有 `--write-yfinance-*` flag、guard 與輸出文案保持相容，`core.py` 已降到 1946 行。 |
...
| 本輪本機驗證 | 2026-05-24 Windows/K：Backend/Core hardening checkpoint `dc7523d` 已驗證。temp `PYTHONPYCACHEPREFIX` `py_compile` 通過；`py -3 -m unittest tests.test_yfinance_adapter tests.test_tk_ui_helpers tests.test_tk_dialogs -v` 共 39 tests OK；`git diff --check` OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26342897778` Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260524_043113.log`，不提交。 |
...
10. Backend/Core Hardening 第一刀已完成 yfinance CLI compatibility slice；下一刀仍只選一個可測邊界：download/import CLI 群組、database-repair CLI 群組，或 `adapter_plan_resolver.py` 的 source-specific resolver 拆分。DB migration runner、真實 subcommands、crawler dynamic discovery 仍需 OpenSpec 先定義相容、rollback 與 CI gate。
