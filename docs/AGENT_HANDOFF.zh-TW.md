...
- `api_launcher/adapter_plan_resolver.py` 是第一個 non-direct plan resolver：CLI `--resolve-adapter-plan INPUT --write-resolved-adapter-plan OUTPUT` 會掃描 CKAN/Data.gov 類候選常見的 `dataset_version.metadata.resources`，只把看起來已經是 direct file URL 的 resource 提升成 direct download plan entry，並重建 `target_path`、`download_eligibility`、`import_plan`。HTML、API selector、登入頁或未知格式仍留在 adapter review；這是 bounded plan rewrite，不是無頭亂爬整站。
...
1. 把 `--resolve-adapter-plan` 暴露到 Tk UI 的下載計畫/Adapter 待辦流程，接著實作第一個 bounded API-query adapter：可從 `adapter_review` queue 選 NOAA ERDDAP query 模板、STAC item search、或其他官方 catalog/API 到 direct plan entries 的轉換。避免一開始就硬寫大型網站流程。
