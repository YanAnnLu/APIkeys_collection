...
- 重要邊界：候選 digest 只代表「當次 crawl 的快照」。`load_crawler_assets()` 不會重爬遠端，所以目前不會標記 `candidate_snapshot_changed`；未來只有 explicit fresh crawl / rebuild plan 後，才應比較新舊 digest 並提示使用者重建。
