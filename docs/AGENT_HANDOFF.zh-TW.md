...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端實質 checkpoint 是 `ed16370 Split Tk sidebar and layout workflows`；GitHub Actions run `26337041547` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第二十四輪安全解耦：把側欄分類 / provider favicon 支援抽成 `SidebarWorkflowMixin`，把細節抽屜動畫 / responsive layout / 表格欄寬重新配置抽成 `ResponsiveLayoutWorkflowMixin`，把 Treeview 欄寬調整、游標、搜尋 placeholder、分類切換事件抽成 `TableInteractionWorkflowMixin`。`launcher_ui.py` 已降到 1389 行。仍沒有搬主表格 reload/filter/render、row action/crawl/install/uninstall 或 database repair backend。 |
