...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪功能 checkpoint 是 `aca3de5 Add UI import policy selection`：Tk UI import policy dialog、replace policy regression test、GTD/handoff 同步已推到 `origin/main`。push 後已用 `gh run watch 26144029986 --repo YanAnnLu/APIkeys_collection --exit-status` 確認 CI success。 |
| 上次本機驗證 | 2026-05-20 macOS/Antigravity/CloudMounter：IDE 更新後目前這個舊 Codex session 仍被 `CODEX_SANDBOX=seatbelt`、`CODEX_SANDBOX_NETWORK_DISABLED=1` 限制；`~/.codex/config.toml` 已是 `sandbox_mode = "danger-full-access"` / `approval_policy = "never"`，但本 session 沒吃到，建議關閉 IDE / reload window / 開新 Codex session 後先驗證環境變數。餘波修復：`APIkeys_collection.py` 曾被工作樹刪除，已用 HEAD 可讀內容補回；缺失 Git blob `a60de8185d6ad444079a35ede668b19b1e70c5fa` 經確認等於 `frontends/unreal/README.zh-TW.md` 工作樹 hash 後，用 `git hash-object -w` 補回。功能驗證：`PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m py_compile APIkeys_collection.py APIkeys_collection_ui.py frontends/tk/launcher_ui.py api_launcher/downloads/plan_runner.py api_launcher/core.py tests/test_download_plan_runner.py` OK；`PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m unittest tests.test_download_plan_runner -v`，7 tests OK；sandbox 內 full test 因 seatbelt 禁止 `127.0.0.1` 測試伺服器 bind 而報 `PermissionError: Operation not permitted`，升權後重跑 `PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m unittest discover -s tests -v`，333 tests OK、skipped=1（optional numpy renderer dependency）；`git diff --check` OK；GitHub Actions run `26144029986` Windows/Ubuntu success。 |
| 最近新增重點 | Tk UI guided import 現在會在「匯入可支援下載結果」前詢問同名 SQLite table 處理方式：預設「保留舊表，匯入成新表」，也可選「保留舊表，略過同名項目」或「覆蓋同名表」；覆蓋會二次確認。這把 CLI 既有 `skip` / `rename` / `replace` policy 接到 UI，保持 beginner-safe 預設不覆蓋。`tests.test_download_plan_runner` 也補了 replace regression：明確 replace 會重建同名表，不會保留舊資料或偷偷改名。上一個 checkpoint：Plan-driven import CLI 可用 `--plan-import-existing-table-policy rename` 把新結果匯入 `table_2` 這類新表，或明確用 `replace` / `--import-replace-table` 覆蓋。 |
...
6. Import policy UI 已有第一版：Tk guided import 現在可選 keep/rename/replace，預設仍是安全 rename，不覆蓋原資料。後續 polish 可考慮記住使用者偏好、改善文案或把策略顯示在下載計畫狀態欄，但不要改掉 beginner-safe 預設。
