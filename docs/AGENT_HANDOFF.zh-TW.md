...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端實質 checkpoint 是 `d47a8fc Split import Tk workflows`；GitHub Actions run `26334490851` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第十九輪安全解耦：把下載後匯入、同名資料表策略、本機 CSV/JSON/JSONL/GeoJSON 檔匯入、匯入狀態標籤與 manifest 健康檢查文案抽成 `ImportWorkflowMixin`；backend importer、manifest、SQLite 安全策略與 `run_existing_download_import_slice()` 行為不變。`launcher_ui.py` 已降到 2771 行。仍沒有搬主表格/detail 控制、crawler/adapter plan UI 小流程或資料庫 repair backend。 |
