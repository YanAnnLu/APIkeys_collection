...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端 checkpoint 是 `1ec684d Split import table policy dialog`；GitHub Actions run `26327044900` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第十輪安全解耦：把匯入支援下載結果前的「既有資料表處理方式」視窗抽成 `ImportExistingTablePolicyDialog` class，主檔只保留委派呼叫，實際匯入 pipeline、table rename/skip/replace 行為與偏好保存仍走既有方法。`launcher_ui.py` 已降到約 5362 行。仍沒有搬 OAuth flow 實作、repair panel、下載/匯入 backend 或資料庫 repair backend。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `1ec684d` 已驗證。`py_compile` 使用 temp `PYTHONPYCACHEPREFIX` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_ui_helpers tests.test_tk_provider_models -v` 共 60 tests OK；`git diff --check` OK；`.\scripts\pre_push_smoke_brief.cmd` 共 508 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26327044900` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260523_153002.log`，不提交。 |
