...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準。最新功能 checkpoint 是 database repair CLI/agent 路徑：real DB smoke 本機指南、manifest-backed SQLite table reimport、以及 registry-only database/table stop-tracking 都已接上 JSON output；前一個 real-db-smoke CI run `26165313576` 已確認 Windows/Ubuntu/real-db-smoke 全部 success。 |
...
| 本輪本機驗證 | 2026-05-20 Windows/K: 補本機 real DB smoke 指南、CLI guarded SQLite table reimport、以及 CLI registry-only stop-tracking 後，`tests.test_database_repair tests.test_install_registry tests.test_database_self_check -v` 共 62 tests OK；`python -m unittest discover -s tests` 共 349 tests OK、skipped=4；`py_compile` OK；`python APIkeys_collection.py --summary` OK；`git diff --check` OK；`gh auth status` 已登入。過程中發現 HEAD 的 `docs/AGENT_HANDOFF.zh-TW.md` blob `beaccfd88d728b3bb72e36251dacaa126b75b3fc` 在本機 object store 缺失，已用 GitHub blob API + `git hash-object -w` 非破壞性補回；工作樹修改未被覆蓋。 |
| 最近新增重點 | Archive bounded transform 現在支援 ZIP/TAR 內的 `csv.gz`、`json.gz`、`jsonl.gz`、`ndjson`、`ndjson.gz`、`geojson.gz`，會寫衍生 manifest 並接既有 CSV/JSON importer；regression 已覆蓋 ZIP 內 `sample.ndjson.gz` 與 TAR.GZ 內 `sample.geojson.gz` 匯入 SQLite。Registry/source provenance 也正式接受並保存 compound source format，例如 `csv.gz`、`jsonl`、`ndjson`、`geojson`、`geojson.gz`、`zip`、`tar.gz`；下載檔案 asset 與 CSV/JSON/GeoJSON 匯入後的 curated table asset 會記錄實際 payload format。Guarded SQLite table reimport 也會用同一組支援格式，且錯誤/UI 文案會列出支援清單；現在 CLI/agent 可用 `--reimport-missing-sqlite-table ASSET_ID --database-repair-json` 走同一個安全 guard，也可用 `--unmanage-database-asset ASSET_ID --database-repair-json` 做 registry-only 停止追蹤。本輪讓 opt-in real MySQL/PostgreSQL smoke tests 進一步覆蓋 registry-backed table self-check：`APIKEYS_REAL_DB_SMOKE_ALLOW_WRITE=1` 時才會建立/清掉 `apikeys_ci_registry_smoke_*` 測試表，確認 managed table asset 的 present/missing 狀態，並 ALTER 同一張 generated table 觸發 schema fingerprint drift error。`docs/SETUP.zh-TW.md` 已補本機 disposable Docker/env-var 指南；一般本機/Windows/Ubuntu 全量測試仍預設不連線、不 DROP、不覆蓋既有 table、也不猜測未記錄來源。 |
