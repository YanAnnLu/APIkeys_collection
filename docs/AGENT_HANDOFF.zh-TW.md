...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪最近功能 checkpoint 是 plan-driven import 新增 CLI safe-rename policy；push 後請用 `gh run list --branch main --limit 5` 與 `gh run watch ... --exit-status` 確認 CI。 |
| 上次本機驗證 | 2026-05-20 Windows/RaiDrive：plan import existing-table policy checkpoint 使用本機磁碟 venv `C:\Users\lyn59\AppData\Local\APIkeys_collection\venv-py313\Scripts\python.exe` 驗證；`-B -m py_compile api_launcher\downloads\plan_runner.py api_launcher\importers\csv_importer.py api_launcher\core.py tests\test_download_plan_runner.py` OK；`-B -m unittest tests.test_download_plan_runner -v`，6 tests OK；`-B -m unittest discover -s tests -v`，332 tests OK、skipped=0；`git diff --check -- api_launcher\downloads\plan_runner.py api_launcher\importers\csv_importer.py api_launcher\core.py tests\test_download_plan_runner.py docs\AGENT_HANDOFF.zh-TW.md docs\PROJECT_GTD.md docs\USER_GUIDE.zh-TW.md docs\DATASET_DISCOVERY_NOTES.zh-TW.md docs\appendices\discovery.zh-TW.md docs\TECH_STACK.md` OK。上一個 checkpoint：dependency/test-environment 使用同一 venv，CI run `26133399043` success。注意：本機 `python` 指到 WindowsApps 佔位程式；雲端磁碟寫 `__pycache__` 或 `.venv` 套件檔可能卡住，測試時可用 `-B`、設 `PYTHONDONTWRITEBYTECODE=1`，或改用本機磁碟 venv/pycache。 |
| 最近新增重點 | Plan-driven import 現在保留安全預設：`--run-download-plan ... --import-supported-plan-results` 遇到既有 SQLite table 時仍預設 `skipped_existing_table`，不覆蓋；power user 可用 `--plan-import-existing-table-policy rename` 把新結果匯入 `table_2` 這類新表，或明確用 `--plan-import-existing-table-policy replace` / `--import-replace-table` 覆蓋。上一個 bounded adapter checkpoint：Generic adapter-plan resource resolver 會辨識 `resources` / `distribution` / `distributions` / `links` / JSON-LD `@graph`、常見 direct-link 欄位與 DCAT/schema.org 命名空間；`accessURL` 這類可能是入口頁的欄位仍不會被泛用 resolver 自動當下載檔。 |
| MVP 剩餘估算 | 約 13-14%；bounded adapter 的 DOI/DataCite、NOAA/NCEI data-file、CMR granule data-link、常見 resource 欄位命名與 JSON-LD `@graph` 包裝風險各收掉一塊，database self-check UI 已有 profile/schema 修正、停止追蹤單一壞資產與 manifest-backed missing SQLite table reimport，plan-driven import 重跑現在有預設略過、CLI safe-rename、明確 replace 三種路徑；剩下主要是擴大 guarded repair 範圍、crawler source 類型收束、以及把 import policy 選擇補進 UI |
...
6. 中期再補 advanced import policy UI：CLI 現在已有預設略過、`--plan-import-existing-table-policy rename`、以及明確 replace；Tk UI guided import 目前仍預設安全改名。下一步若要 polish，應把 keep/rename/replace 選擇顯示在 UI，而不是改變預設不覆蓋原則。
