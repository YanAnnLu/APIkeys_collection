...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端 checkpoint 是 `2827056 Split provider candidate review dialog`；GitHub Actions run `26329248495` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第十三輪安全解耦：把 provider 候選審核視窗抽成 `ProviderCandidateReviewDialog` class，主檔只保留讀取 `state/provider_candidates.ui.json` 與開窗委派；candidate 仍只能寫 ignored local provider seed / dataset source draft，不會直接改正式 catalog，source draft 仍需 local discovery audit 後才可 promotion。`launcher_ui.py` 已降到約 5187 行。仍沒有搬 OAuth flow 實作、repair panel、下載/匯入 backend 或資料庫 repair backend。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `2827056` 已驗證。`py_compile` 使用 temp `PYTHONPYCACHEPREFIX` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui -v` 共 42 tests OK；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_ui_helpers tests.test_tk_provider_models -v` 共 62 tests OK；`git diff --check` OK；`.\scripts\pre_push_smoke_brief.cmd` 共 510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26329248495` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260523_172144.log`，不提交。 |
