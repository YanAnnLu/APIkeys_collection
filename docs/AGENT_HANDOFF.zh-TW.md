...
| --- | --- |
| Branch | `main` |
| 最新已驗證 checkpoint | 2026-05-25 Windows/K：`b23f135 Centralize crawler asset bound facets` 已推送，GitHub Actions run `26396017212` 成功。這是 Crawler Asset Bounds 小切片：`bounds_facets_for_source()` 已從 source_type if 鏈收斂為 `SOURCE_BOUND_FACETS` registry，Tk/Qt/CLI 可共用同一份界域表單契約；`ogc_wms_capabilities` 現在也有 collection / bbox / time / format / limit 界域，不再退回只有 `limit`。前兩個 source draft registry checkpoint `c31dc41`、`cbb9deb` 也已成功並寫入 development log。 |
| 最新本機驗證 | 2026-05-25 Windows/K：crawler asset bounds registry 已驗證。`b23f135` 本機驗證：crawler asset/source pattern/discovery targeted tests 73 tests OK；`py_compile` OK；`git diff --check` OK；完整 `py -B -m unittest discover -s tests` 624 tests OK、skipped=4；`scripts\pre_push_smoke_brief.cmd` 通過：624 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`。完整本機 log 在 `state/logs/pre_push_smoke_20260525_182917.log`，不提交。 |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。最新實質功能 checkpoint 是 `b23f135 Centralize crawler asset bound facets`，CI run `26396017212` 成功；後續若只有 log / handoff 同步 commit，請視為文件狀態更新，不代表新的功能切片。最新文件修復 commit `3e65459 Fix source draft log encoding`、handoff 同步 commit `ae87c31 Update source draft handoff status` 也已通過 CI。 |
