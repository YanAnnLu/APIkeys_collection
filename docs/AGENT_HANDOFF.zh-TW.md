...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-24 目前已確認的遠端實質 checkpoint 是 `98a42b2 Split download plan CLI workflow`；GitHub Actions run `26343407952` 成功。這是 Backend/Core Hardening 的第三個小切片：`api_launcher/cli_download_plan.py` 接手 `--run-download-plan` / `--run-download-plan-json` / `--import-supported-plan-results` 的 CLI 外殼與 `download_plan_executed` event；`core.py` 只保留 `run_download_plan_cli(self.args, self.repository, log_event)` 調度點。實際 download/import pipeline 仍由 `api_launcher/ingestion_pipeline.py` 擁有，既有 skip/next_action、plan-driven import、table policy 與 JSON schema 保持不變，`core.py` 已降到 1798 行。 |
...
| 本輪本機驗證 | 2026-05-24 Windows/K：Backend/Core hardening checkpoint `98a42b2` 已驗證。temp `PYTHONPYCACHEPREFIX` `py_compile` 通過；`py -3 -m unittest tests.test_download_plan_runner tests.test_ingestion_pipeline tests.test_repair -v` 共 26 tests OK；`git diff --check` / cached check OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26343407952` Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260524_045616.log`，不提交。 |
...
10. Backend/Core Hardening 已完成 yfinance CLI compatibility slice、database-repair CLI slice 與 download-plan CLI slice；下一刀仍只選一個可測邊界：manual/local-file import CLI 群組，或 `adapter_plan_resolver.py` 的 source-specific resolver 拆分。DB migration runner、真實 subcommands、crawler dynamic discovery、credential catalog 化仍需 OpenSpec 先定義相容、rollback 與 CI gate。
