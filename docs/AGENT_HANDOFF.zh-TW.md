...
- Web Preview crawler asset card/detail 現在也會從近期 `crawler_asset_plan_outcome_recorded` event 讀回 compact `plan_passport`。`frontends/web/preview_api.py` 只允許白名單欄位進入前端 payload，完整 `providers` / resolved plan body 不會從 event log 回灌到 UI；因此頁面重載後的「下載器」分頁可顯示真實計畫護照摘要，但正式長期狀態仍應回到 asset profile / backend service。
