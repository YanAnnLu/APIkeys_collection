...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端實質 checkpoint 是 `bdad2db Split provider settings Tk workflows`；GitHub Actions run `26336615256` rerun 後成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第二十三輪安全解耦：把新增/編輯 provider、provider row 轉回 `core.Provider`、資料庫工具/設定入口、integration config 開啟、文件開啟、Developer CLI、語言/AI/data-store/startup/event/Gemini 設定 dialog 入口抽成 `ProviderSettingsWorkflowMixin`。`PROJECT_ROOT` 仍保留在 `launcher_ui.py` 作為既有 import 相容面；`launcher_ui.py` 已降到 1628 行。仍沒有搬主表格/detail 控制或資料庫 repair backend。 |
