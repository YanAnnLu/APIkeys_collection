...
| --- | --- |
| Branch | `main` |
| 最新已驗證 checkpoint | 2026-05-24 Windows/K：`48bdbfb Split NCEI access data resolver` 已推送，GitHub Actions run `26346575874` 成功。這是 Backend/Core Hardening 第十五個小切片：`api_launcher/adapter_plan_resolvers/ncei_access_data.py` 接手 NOAA/NCEI Access Data bounded CSV/JSON sample resolver；主檔保留相容 wrapper，`adapter_plan_resolver.py` 從 2874 行降到 1272。 |
| 最新本機驗證 | temp `PYTHONPYCACHEPREFIX` `py_compile` 通過；`py -3 -m unittest tests.test_adapter_plan_resolver tests.test_dataset_download_plan -v` 共 63 tests OK；`git diff --check` OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`。完整本機 log 在 `state/logs/pre_push_smoke_20260524_073726.log`，不提交。 |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-24 目前已確認的遠端實質 checkpoint 是 `48bdbfb Split NCEI access data resolver`；GitHub Actions run `26346575874` 成功。這是 Backend/Core Hardening 的第十五個小切片，接在 DataCite / Dataverse / CKAN / Socrata / CMR granule asset / CMR collection / STAC / ERDDAP / NCEI Search / NCEI data-file resolver 拆分後，NCEI 相關 resolver 三段已全部離開主檔。 |
