...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-24 目前已確認的遠端實質 checkpoint 是 `7337f36 Split Tk table data workflows`；GitHub Actions run `26337375378` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第二十五輪安全解耦：把下載計畫面板建置與收合/展開狀態抽成 `DownloadPlanPanelWorkflowMixin`，把 provider/dataset 表格 reload/filter/render 與 dataset row helper 抽成 `TableDataWorkflowMixin`。`launcher_ui.py` 已降到 1116 行。仍沒有搬 row action/crawl/install/uninstall、修復面板 backend 或 database repair backend；這些留給下一輪較高風險切片。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `bdad2db` 已驗證。`py_compile` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_helpers tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_provider_models -v` 共 62 tests OK；`git diff --check` / cached check OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26336615256` 首輪 Windows checkout credential transient failure，未進測試，`gh run rerun --failed` 後 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260523_232756.log`，不提交。 |
