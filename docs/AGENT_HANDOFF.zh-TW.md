...
| --- | --- |
| Branch | `main` |
| 最新已驗證 checkpoint | 2026-05-24 Windows/K：`d281e16 Split DataCite and Dataverse plan resolvers` 已推送，GitHub Actions run `26344386999` 成功。這是 Backend/Core Hardening 第六個小切片：新增 `api_launcher/adapter_plan_resolvers/`，把 DataCite DOI contentUrl resolver 與 Dataverse latest-version file resolver 移到來源專屬模組；`api_launcher/adapter_plan_resolver.py` 保留相容 wrapper 與既有 `fetch_json` patch 點，行數從 2874 降到 2572。 |
| 最新本機驗證 | temp `PYTHONPYCACHEPREFIX` `py_compile` 通過；`py -3 -m unittest tests.test_adapter_plan_resolver -v` 共 46 tests OK；`git diff --check` OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`。完整本機 log 在 `state/logs/pre_push_smoke_20260524_054223.log`，不提交。 |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-24 目前已確認的遠端實質 checkpoint 是 `d281e16 Split DataCite and Dataverse plan resolvers`；GitHub Actions run `26344386999` 成功。這是 Backend/Core Hardening 的第六個小切片：先建立 `api_launcher/adapter_plan_resolvers/` 作為來源專屬 resolver 拆分位置，並把 DataCite / Dataverse 兩組二階段 metadata lookup 移入獨立模組。 |
...
| 本輪本機驗證 | 2026-05-24 Windows/K：Backend/Core hardening checkpoint `d281e16` 已驗證。temp `PYTHONPYCACHEPREFIX` `py_compile` 通過；`py -3 -m unittest tests.test_adapter_plan_resolver -v` 共 46 tests OK；`git diff --check` / cached check OK；`.\scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26344386999` Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260524_054223.log`，不提交。 |
