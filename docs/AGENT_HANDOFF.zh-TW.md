...
| --- | --- |
| Branch | `main` |
| 最新已驗證 checkpoint | 2026-05-25 Windows/K：`15ed86a Normalize Socrata catalog draft queries` 已推送，GitHub Actions run `26389482453` 成功。這是 Discovery / Socrata Source Draft Hardening 小切片：Socrata canonical catalog endpoint 現在只保留必要的 `domains=<portal-host>`，移除 probe pagination、搜尋字串與 fragment，避免 local source draft 被一次性探測參數污染。最新 log-only handoff/history 同步 commit 是 `cfc7188 Log Socrata catalog draft query checkpoint`，GitHub Actions run `26389578001` 也成功。 |
| 最新本機驗證 | 2026-05-25 Windows/K：targeted discovery/source-pattern tests 72 tests OK；`py_compile` OK；`git diff --check` 無錯誤但保留 docs CRLF warning；`scripts\pre_push_smoke_brief.cmd` 通過：621 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`。完整本機 log 在 `state/logs/pre_push_smoke_20260525_154125.log`，不提交。 |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。目前遠端 HEAD 是 `cfc7188 Log Socrata catalog draft query checkpoint`；最新實質功能 checkpoint 是 `15ed86a Normalize Socrata catalog draft queries`，CI run `26389482453` 成功。前一個實質 checkpoint `cec6dfe Harden search source draft endpoints` 也已記錄到 development log，CI run `26389032248` 成功。 |
