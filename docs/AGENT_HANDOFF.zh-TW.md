...
- Crawler asset bounds 的 `version` 欄位已接到真實版本選擇：`source_download_options_from_crawler_asset_payload()` 會把 source-level version facet 轉成 `selected_versions["*"]`，`selected_version_options()` 會套用到 crawler 回傳的 dataset。這是暫時的 source-level selector；未來 dataset-specific UI 可以覆蓋它，不要再把版本文字誤塞成 `version_limit`。
