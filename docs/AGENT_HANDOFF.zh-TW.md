...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端 checkpoint 是 `7a39951 Add provider candidate source draft CLI`；GitHub Actions run `26322334358` 成功。這個 checkpoint 讓 CLI/agent 可把 provider candidate review JSON 批次轉成 ignored local dataset discovery source 草稿，並輸出 summary / provider filter；正式 catalog 仍需 local discovery dry-run audit / crawler audit / promotion guard。 |
