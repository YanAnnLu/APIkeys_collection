...
- Tk UI 的 Gemini/Google dialog 已改成比較誠實的狀態面板：它會直接說明 AI 生成管線已存在，但需要 local Ollama、Gemini API key 或 OAuth token 才能真正呼叫模型；貼 API key、QR 登入、AI 模型設定與產生目前資料源描述的按鈕會固定顯示。
