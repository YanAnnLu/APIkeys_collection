...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端 checkpoint 是 `a601942 Split MVP demo Tk workflows`；GitHub Actions run `26333075944` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第十五輪安全解耦：把 MVP Demo Flow 建立、canonical MVP Demo Smoke 啟動、背景 worker 與結果 callback 抽成 `MvpDemoWorkflowMixin`，主檔只保留工具選單 callback、主視窗狀態與繼承 mixin；demo backend、下載/匯入 pipeline、隔離 demo DB 與 event log 邊界不變。`launcher_ui.py` 已降到約 4821 行。仍沒有搬 OAuth flow 實作、repair panel、下載/匯入 backend 或資料庫 repair backend。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `a601942` 已驗證。`py_compile` 使用 temp `PYTHONPYCACHEPREFIX` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_helpers -v` 共 47 tests OK；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_ui_helpers tests.test_tk_provider_models -v` 共 62 tests OK；`git diff --check` / cached check OK；`.\scripts\pre_push_smoke_brief.cmd` 共 510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26333075944` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260523_204142.log`，不提交。 |
