...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端實質 checkpoint 是 `fd895ec Split download Tk workflows`；GitHub Actions run `26333973925` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第十八輪安全解耦：把下載計畫面板、直接下載啟動、skip guidance、download queue 狀態控制、progress callback、完成後 manifest/file asset 註冊流程抽成 `DownloadWorkflowMixin`；下載 queue / downloader / manifest backend 行為不變，主檔只保留計畫資料模型與匯入流程。`launcher_ui.py` 已降到 2936 行。仍沒有搬匯入 workflow、主表格/detail 控制、crawler/adapter plan UI 小流程或資料庫 repair backend。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `fd895ec` 已驗證。`py_compile` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_helpers tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_provider_models tests.test_repair -v` 共 73 tests OK；`git diff --check` / cached check OK；`.\scripts\pre_push_smoke_brief.cmd` 共 510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26333973925` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整本機 log 在 `state/logs/pre_push_smoke_20260523_212650.log`，不提交。第一次 `git push origin main` 曾因 Git 暫時讀不到 current working directory 失敗；已改用 `git -C K:\APIkeys_collection push origin main` 成功，未改動工作樹內容。 |
