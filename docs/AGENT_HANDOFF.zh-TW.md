...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪最近功能 checkpoint 是 CMR granule metadata/data-link resolver guard；push 後請用 `gh run list --branch main --limit 5` 與 `gh run watch ... --exit-status` 確認 CI。 |
| 上次本機驗證 | 2026-05-19 Windows/CloudMounter：CMR granule link guard checkpoint 使用 `PYTHONDONTWRITEBYTECODE=1 .\.venv\Scripts\python.exe -m unittest tests.test_adapter_plan_resolver tests.test_dataset_download_plan -v`，40 tests OK；`PYTHONDONTWRITEBYTECODE=1 .\.venv\Scripts\python.exe -m unittest discover -s tests -v`，304 tests OK、skipped=1；`git diff --check` OK。注意：本機 `python` 指到 WindowsApps 佔位程式，請用 `.venv\Scripts\python.exe`；雲端磁碟寫 `__pycache__` 可能卡住，測試時建議設 `PYTHONDONTWRITEBYTECODE=1` 或把 pycache 指到本機暫存。前一個 CMR checkpoint：CMR live smoke 抓到 6 candidates 並能轉成 `cmr_bounded_granule_search_resolver` 的 `/search/granules.json?collection_concept_id=...&page_size=1`。 |
| 最近新增重點 | `api_launcher/adapter_plan_resolver.py` 現在會在 CMR candidate 裡跳過 `metadata/documentation/browse/service/opendap/self/...` 這些 metadata 或服務入口，不會因為它們看似 `.json` 就提升成 direct download；真正 `rel=data` / `rel=download` / `rel=enclosure` 的直接檔案連結仍可由 generic resolver 提升，NetCDF/HDF 這類科學檔會下載為 direct asset，但 SQLite 匯入仍會標成 manual review。前一個重點：OGC API Records candidate 會跳過 `self/alternate/canonical/describedby/items/related/...` metadata/navigation links；真正 `rel=data` / `rel=download` 類直接資料連結仍可由 generic resolver 提升。OpenSpec CLI 已初始化 `openspec/`，Spectra GUI 已安裝到 `~/Applications/Spectra.app`，Qt Designer 已確認在 `metal_trade_312`；crawler source type dispatcher 已集中成 `SOURCE_CRAWLER_HANDLERS` mapping，portal intake 共用同一份 `SUPPORTED_DATASET_SOURCE_TYPES`；跨平台接力檢查已併入本文件 |
