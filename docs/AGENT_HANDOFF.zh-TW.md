...
- Generic resource resolver 的 URL suffix 推論已改用 format 正規化；即使 metadata 沒有 mediaType/format hint，`.nc`、`.cdf`、`.h5`、`.hdf5` 與 `.gpkg` 這類清楚檔案 URL 也可進 direct plan，再由 content parser review 擋住 curated import。
...
- Direct download eligibility 已補 `.gpkg` 與 `.cdf`，所以 GeoPackage 與舊式 NetCDF 檔案型 URL 不會在舊 provider/direct URL 判斷路徑被誤導成 adapter required。
...
- HTML file index detector 本身也已同步辨識複合壓縮與地理/氣象資料檔連結，例如 `.geojson.gz`、`.cdf`、`.hdf5`、`.gpkg`、`.zarr`、`.grib2`、`.tar.gz`、`.csv.zst`；只有這類檔案的入口頁不應再因舊副檔名清單而掉到低信心 `unknown`。
- HTML file index 預設 regex 已覆蓋 `.csv.gz`、`.csv.zst`、`.geojson.gz`、`.cdf`、`.hdf5`、`.gpkg`、`.zarr`、`.grib2`、`.tar.gz` 等常見資料檔，避免 source draft 後續 audit 只因壓縮副檔名回傳零候選。
...
| --- | --- |
| Branch | `main` |
| 最新已驗證 checkpoint | 2026-05-24 Windows/K：`9d30994 Split resource format helpers` 已推送，GitHub Actions run `26347045573` 成功。這是 Backend/Core Hardening 第十七個小切片：`api_launcher/adapter_plan_resolvers/resource_formats.py` 接手 format/mediaType normalization 與 compound suffix 推論；主檔保留相容 wrapper，目前 `adapter_plan_resolver.py` 實測約 1385 行。 |
| 最新本機驗證 | `py -3 -m py_compile api_launcher\adapter_plan_resolver.py api_launcher\adapter_plan_resolvers\resource_formats.py` 通過；`py -3 -m unittest tests.test_adapter_plan_resolver tests.test_dataset_download_plan -v` 共 63 tests OK；`git diff --check` OK；`cmd /c scripts\pre_push_smoke_brief.cmd` 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`。完整本機 log 在 `state/logs/pre_push_smoke_20260524_080246.log`，不提交。 |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-24 目前已確認的遠端實質 checkpoint 是 `9d30994 Split resource format helpers`；GitHub Actions run `26347045573` 成功。這是 Backend/Core Hardening 的第十七個小切片，接在 DataCite / Dataverse / CKAN / Socrata / CMR granule asset / CMR collection / STAC / ERDDAP / NCEI Search / NCEI data-file / NCEI Access Data resolver 與 CMR/OGC metadata guard 拆分後，將 resource format 邊界也移出主檔。 |
