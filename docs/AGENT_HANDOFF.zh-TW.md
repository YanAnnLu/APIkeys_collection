...
| MVP 剩餘估算 | 約 23-27%；剩下主要是 bounded API/query adapter 擴充、database self-check UI/repair action、crawler source 類型擴充、import policy 與少量 UI polish |
...
- `api_launcher/adapter_plan_resolver.py` 現在也能把 NOAA/NCEI Common Access Search entry 轉成 bounded JSON metadata sample。若 plan 有 `/search/v1/datasets` 且 metadata 裡有 NCEI dataset id，會改成 `/search/v1/data?dataset=...&limit=25&offset=0`；若 plan 本來就是 `/search/v1/data`，會把 limit/offset 壓到 25/0。這只保存搜尋結果 metadata，不下載 NOAA 原始資料檔。
...
1. 擴充 bounded API-query adapters：CKAN package metadata lookup、ERDDAP sample resolver、STAC `limit=1` item metadata GeoJSON sample、Socrata/SODA `$limit=25` sample、NOAA/NCEI Search `limit=25&offset=0` metadata sample 已先打通五條小閉環；下一步可補更細的 NOAA/NCEI data-file selector，仍要從 crawler/adapter_review handoff 進入，不要硬寫代表資料集。
...
3. 繼續擴充 crawler source 類型，但要維持設定檔驅動；Dataverse/Zenodo/DataCite/OGC API Records/Socrata 已有 metadata parser 與 pagination flow，下一批可評估 OpenAlex 或更細的 NOAA/NCEI data-file selector。
