...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git log -1 --oneline` 為準；不要把本欄位當成長期有效 SHA。本輪功能 checkpoint 是 source provenance / guarded reimport：registry 會正式保存 compound source format，下載檔案與 curated table asset 會記錄實際 payload format，Tk repair/CLI 文案也會顯示 CSV/JSON/GeoJSON 支援範圍。push 後請用 `gh run list --repo YanAnnLu/APIkeys_collection --branch main --limit 5` 與 `gh run watch RUN_ID --repo YanAnnLu/APIkeys_collection --exit-status` 確認 CI。 |
| 上次本機驗證 | 2026-05-20 macOS/Antigravity/CloudMounter：IDE 更新後目前這個舊 Codex session 仍被 `CODEX_SANDBOX=seatbelt`、`CODEX_SANDBOX_NETWORK_DISABLED=1` 限制；`~/.codex/config.toml` 已是 `sandbox_mode = "danger-full-access"` / `approval_policy = "never"`，但本 session 沒吃到，建議關閉 IDE / reload window / 開新 Codex session 後先驗證環境變數。餘波修復：`APIkeys_collection.py` 曾被工作樹刪除，已用 HEAD 可讀內容補回；缺失 Git blob `a60de8185d6ad444079a35ede668b19b1e70c5fa` 經確認等於 `frontends/unreal/README.zh-TW.md` 工作樹 hash 後，用 `git hash-object -w` 補回。本輪 source provenance / guarded reimport 驗證：`PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m py_compile api_launcher/provenance.py api_launcher/repository.py api_launcher/importers/csv_importer.py api_launcher/importers/json_importer.py api_launcher/database_repair.py api_launcher/core.py frontends/tk/launcher_ui.py tests/test_database_repair.py tests/test_json_importer.py tests/test_manifest_registry.py` OK；`PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m unittest tests.test_database_repair tests.test_json_importer tests.test_csv_importer tests.test_manifest_registry tests.test_install_registry -v`，38 tests OK；sandbox 內 full test 因 seatbelt 禁止 `127.0.0.1` 測試伺服器 bind 仍需升權，升權後 `PYTHONPYCACHEPREFIX=/tmp/apikeys_collection_pycache python3 -m unittest discover -s tests -v`，338 tests OK、skipped=1（optional numpy renderer dependency）；`git diff --check` exit code 0，但仍會提示 `api_launcher/repository.py` 下次 Git touch 時 CRLF 會轉 LF。 |
| 最近新增重點 | Registry/source provenance 現在正式接受並保存 compound source format，例如 `csv.gz`、`jsonl`、`ndjson`、`geojson`、`geojson.gz`、`zip`、`tar.gz`；下載檔案 asset 與 CSV/JSON/GeoJSON 匯入後的 curated table asset 會記錄實際 payload format。Guarded SQLite table reimport 也會用同一組支援格式，且錯誤/UI 文案會列出支援清單。這仍只針對 registry 裡明確納管、狀態為 missing/error、notes 有 manifest path、且目標 table 目前不存在的 SQLite table asset；不會 DROP、覆蓋既有 table 或猜測未記錄來源。上一個 checkpoint：Tk UI guided import 現在可選 keep/rename/replace，預設安全 rename，會記住使用者偏好並顯示目前匯入策略。 |
