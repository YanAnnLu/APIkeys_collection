...
| --- | --- |
| Branch | `main` |
| 最新已推送 commit | 接力前請以 `git pull --ff-only origin main`、`git status --short --branch`、`git log -1 --oneline` 為準。2026-05-23 目前已確認的遠端實質 checkpoint 是 `d47a8fc Split import Tk workflows`；GitHub Actions run `26334490851` 成功。這個 checkpoint 是 `frontends/tk/launcher_ui.py` 的第十九輪安全解耦：把下載後匯入、同名資料表策略、本機 CSV/JSON/JSONL/GeoJSON 檔匯入、匯入狀態標籤與 manifest 健康檢查文案抽成 `ImportWorkflowMixin`；backend importer、manifest、SQLite 安全策略與 `run_existing_download_import_slice()` 行為不變。`launcher_ui.py` 已降到 2544 行。仍沒有搬主表格/detail 控制、crawler/adapter plan UI 小流程或資料庫 repair backend。 |
...
| 本輪本機驗證 | 2026-05-23 Windows/K：UI 解耦 checkpoint `d47a8fc` 已驗證。`py_compile` 通過；`py -3 -m unittest tests.test_tk_dialogs tests.test_launcher_ui tests.test_tk_ui_helpers tests.test_tk_ui_labels tests.test_tk_ui_config tests.test_tk_desktop_integration tests.test_tk_provider_models tests.test_ingestion_pipeline -v` 共 66 tests OK；`git diff --check` / cached check OK；第一次 `.\scripts\pre_push_smoke_brief.cmd` 在 510 tests OK 後遇到 Windows 暫時性 `APIkeys_collection.py` permission denied，隨後 `py -3 APIkeys_collection.py --summary` 成功且重跑 pre-push 通過：510 tests OK、skipped=4，MVP demo smoke `stage=download_import_completed`、`row_count=3`；CI run `26334490851` 的 Ubuntu、`windows-2025-vs2026`、real DB smoke 全部 success。完整通過 log 在 `state/logs/pre_push_smoke_20260523_215023.log`，不提交。 |
