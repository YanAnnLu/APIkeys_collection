...
- Tk UI 的 Gemini/Google dialog 已改成比較誠實的狀態面板：它會直接說明 AI 生成管線已存在，但需要 local Ollama、Gemini API key 或 OAuth token 才能真正呼叫模型；貼 API key、設定 QR 登入、QR 登入、AI 模型設定與產生目前資料源描述的按鈕會固定顯示。
- Tk UI 現在有 `整合 > Google QR 登入` 直達入口。若缺 Google OAuth Client ID，QR dialog 會顯示 `設定 QR 登入`，讓使用者貼上 Client ID 並寫入 `config/launcher_integrations.local.json`；`api_launcher/oauth_device.py` 仍保留環境變數 fallback。
