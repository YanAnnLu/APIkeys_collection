...
- Data store connection testing 已有骨架：`--test-data-store PROFILE_ID|all` 可測 configured profiles；`--test-data-store-json` 可輸出 agent-readable status/details/next_action；`--write-data-store-env-template PATH --data-store-env-template-profile PROFILE_ID` 可寫出空值 `.env` 範本，Tk `整合 > 資料儲存連線 > 寫出 env 範本` 走同一個後端。SQLite 會用 read-only introspection，MySQL/PostgreSQL 先檢查 env vars 與 optional Python driver；範本只列變數名稱，不保存密碼。
