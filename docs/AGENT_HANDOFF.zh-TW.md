...
- 已驗證：`py -3 -B -m py_compile api_launcher\crawler_asset_review_display.py api_launcher\crawler_asset_display.py frontends\tk\adapter_review_dialog.py tests\test_tk_dialogs.py` OK；`py -3 -B -m unittest tests.test_tk_dialogs tests.test_web_preview -v` 通過，180 tests OK；`git diff --check` OK；`api_launcher` / `frontends` / `tests` mojibake scan OK；完整 smoke `state\logs\pre_push_smoke_20260530_152157.log` 通過，980 tests / 4 skipped，MVP demo `download_import_completed` / `row_count=3`。
