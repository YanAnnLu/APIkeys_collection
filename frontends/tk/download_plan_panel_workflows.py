...
class DownloadPlanPanelWorkflowMixin:
    ...
    def _build_download_plan_panel(self, parent: ttk.Frame, outer_pad: int) -> None:
        ...
        ttk.Button(header, textvariable=self.download_primary_action_var, style="Action.TButton", command=self.toggle_primary_download_action).pack(side=RIGHT, padx=(8, 0))
        ttk.Button(header, text=self.tr("界域", "Bounds"), style="Action.TButton", command=self.configure_selected_plan_bounds_from_ui).pack(side=RIGHT, padx=(8, 0))
